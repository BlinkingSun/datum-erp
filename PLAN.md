...
| NUC (Linux, `ssh cnc`) | Linux CI node, single tenant (also hosts the live CNC node: node lock, no third-party apt repos). | Ubuntu 26.04: PostgreSQL **18** (the OS's own; the contract pins 17 as the minimum, so the NUC is the cross-major check), `just`, rustup 1.98.1 user-local, sqlx-cli 0.9.0, no docker. Provisioned 2026-09-12. Bootstrap over the unix socket in sqlx's form: `postgres://<unix-user>@%2Fvar%2Frun%2Fpostgresql/postgres` (the libpq `?host=` form is rejected by sqlx). |
| Shop PC (Windows 11, `fireb@192.168.200.98`, cmd.exe over ssh) | Windows CI node, single tenant (node lock, `--windows`). | rustup 1.98.1 with rustfmt/clippy, `just` (winget), PostgreSQL **17.11** from the EDB binaries zip at `C:\pg17` registered as service `postgresql-x64-17` (superuser `postgres`, password in the orchestrator's local env file only). Provisioned 2026-09-12. No sqlx-cli yet. |
