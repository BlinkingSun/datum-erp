...
-- a superuser (see datum_audit::install_privileged); CREATE EVENT TRIGGER is
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'audit') THEN
    EXECUTE 'CREATE SCHEMA audit AUTHORIZATION datum_owner';
  ELSE
    EXECUTE 'ALTER SCHEMA audit OWNER TO datum_owner';
  END IF;
END
$schema$;

REVOKE ALL ON SCHEMA audit FROM PUBLIC;
GRANT USAGE ON SCHEMA audit TO datum_app, datum_audit_row, datum_audit_event,
  datum_migrate, datum_owner;
GRANT CREATE ON SCHEMA audit TO datum_owner, datum_migrate;
...
ALTER TYPE audit.context OWNER TO datum_owner;
GRANT USAGE ON TYPE audit.context TO datum_app, datum_audit_row, datum_audit_event,
  datum_migrate, datum_owner;
...
DECLARE
  ...
  kind text;
BEGIN
  c.actor_id := audit.guc_uuid('datum.actor_id');
  kind := lower(nullif(current_setting('datum.actor_kind', true), ''));
  c.actor_kind := CASE kind
    ...
    ELSE kind
  END;
  c.actor_display := coalesce(nullif(current_setting('datum.actor_display', true), ''), '');
  c.acting_for_id := audit.guc_uuid('datum.acting_for');
  c.session_id := audit.guc_uuid('datum.session_id');
  c.request_id := audit.guc_uuid('datum.request_id');
  c.source_kind := nullif(current_setting('datum.source_kind', true), '');
  c.source_device_id := nullif(current_setting('datum.source_device', true), '');
  c.source_ip := audit.guc_inet('datum.source_ip');
  c.client_app := nullif(current_setting('datum.client_app', true), '');
  c.action := nullif(current_setting('datum.action', true), '');
  c.reason := nullif(current_setting('datum.reason', true), '');
  c.doc_type := nullif(current_setting('datum.doc_type', true), '');
  c.doc_id := audit.guc_uuid('datum.doc_id');
  c.esign_id := audit.guc_uuid('datum.esign_id');
  c.app_version := coalesce(current_setting('datum.app_version', true), '');
  c.config_version := coalesce(current_setting('datum.config_version', true), '');
...
BEGIN
  IF kind IS NULL THEN
    RAISE EXCEPTION 'datum: log_event requires a kind' USING ERRCODE = '42501';
  END IF;
  IF kind !~ '^(security|login|export|print|signature|clock|maintenance|kernel)(\.|$)' THEN
    RAISE EXCEPTION 'datum: log_event kind % is not a kernel event', kind
      USING ERRCODE = '42501';
  END IF;

  IF nullif(current_setting('datum.actor_id', true), '') IS NULL
    ...
    ctx.doc_id := audit.guc_uuid('datum.doc_id');
    IF ctx.doc_id IS NULL AND p_doc_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
      ctx.doc_id := p_doc_id::uuid;
    END IF;
    ctx.esign_id := NULL;
    ctx.app_version := coalesce(current_setting('datum.app_version', true), '');
    ctx.config_version := coalesce(current_setting('datum.config_version', true), '');
...
BEGIN
  ...
  LOOP
    ...
    -- D-2b-12: skip set from datum.schema_class, not a literal name list.
    ...
    CONTINUE WHEN cmd.schema_name = 'datum'
                 OR EXISTS (
                      SELECT 1
                        FROM datum.schema_class sc
                       WHERE sc.nspname = cmd.schema_name::name
...
DECLARE
  ...
  rel_name text;
BEGIN
  -- Superuser-only. Membership in datum_owner grants nothing: datum_owner cannot
  ...
  IF TG_EVENT = 'ddl_command_start' THEN
    ...
    THEN
      RAISE EXCEPTION 'datum: refusing DROP TRIGGER of an audit trigger by non-superuser %',
        session_user USING ERRCODE = '42501';
    END IF;
    IF TG_TAG = 'ALTER TRIGGER' AND q ~* 'zz_audit' THEN
      RAISE EXCEPTION 'datum: refusing ALTER TRIGGER of an audit trigger by non-superuser %',
        session_user USING ERRCODE = '42501';
    END IF;
    IF TG_TAG = 'DROP EVENT TRIGGER'
       AND q ~* 'audit_attach|audit_protect'
    THEN
      RAISE EXCEPTION 'datum: refusing DROP EVENT TRIGGER by non-superuser %',
        session_user USING ERRCODE = '42501';
    END IF;
    ...
    THEN
      RAISE EXCEPTION 'datum: refusing DROP FUNCTION of an audit function by non-superuser %',
        session_user USING ERRCODE = '42501';
    END IF;
    ...
    -- A module's own named trigger stays manageable by datum_migrate.
    IF TG_TAG = 'ALTER TABLE' AND (
      ...
      IF q ~* 'zz_audit' THEN
        RAISE EXCEPTION
          'datum: refusing ALTER TABLE ... TRIGGER by non-superuser %',
          session_user USING ERRCODE = '42501';
      END IF;
  ...
  IF TG_EVENT = 'sql_drop' THEN
    FOR obj IN SELECT * FROM pg_event_trigger_dropped_objects() LOOP
      ...
      THEN
        RAISE EXCEPTION 'datum: refusing DROP TRIGGER %', obj.object_identity
          USING ERRCODE = '42501';
      END IF;
      ...
      THEN
        RAISE EXCEPTION 'datum: refusing DROP FUNCTION %', obj.object_identity
          USING ERRCODE = '42501';
      END IF;
      IF obj.original AND obj.object_type = 'event trigger'
         AND obj.object_name IN ('audit_attach', 'audit_protect', 'audit_protect_drop')
      THEN
        RAISE EXCEPTION 'datum: refusing DROP EVENT TRIGGER %', obj.object_name
          USING ERRCODE = '42501';
      END IF;
    END LOOP;
  END IF;
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $audit$
BEGIN
  EXECUTE format('GRANT INSERT ON TABLE %s TO datum_audit_row', rel);
  EXECUTE format(
    'GRANT INSERT (
       ...
       app_version, config_version
     ) ON TABLE %s TO datum_audit_event',
    rel);
END
$audit$;
  ...
  FOR i IN 0..span LOOP
    ...
    IF NOT EXISTS (
      ...
      EXECUTE format(
        ...
        part, from_ts, to_ts);
      EXECUTE format('ALTER TABLE audit.%I OWNER TO datum_owner', part);
    END IF;
  END LOOP;
...
BEGIN
  ...
  ELSE
    ...
    IF prev_h IS NULL THEN
      RAISE EXCEPTION 'datum: reseal_from missing seq %', from_seq - 1;
    END IF;
  END IF;
...
BEGIN
  RAISE EXCEPTION 'datum: audit.event is insert-only'
    USING ERRCODE = '42501';
END
$audit$;
...
BEGIN
  ...
  ALTER TABLE audit.redact OWNER TO datum_owner;
  ALTER TABLE audit.reason_policy OWNER TO datum_owner;
  ALTER TABLE audit.exempt OWNER TO datum_owner;
  ALTER TABLE audit.tx_seal OWNER TO datum_owner;
  ALTER TABLE audit.anchor OWNER TO datum_owner;
  FOR r IN
  ...
  LOOP
    EXECUTE format('ALTER TABLE %s OWNER TO datum_owner', r.rel);
  END LOOP;
END
$owner$;
...
GRANT INSERT (
  ...
  source_ip, client_app, action, reason, doc_type, doc_id, esign_id,
  app_version, config_version
) ON audit.event TO datum_audit_event;

GRANT SELECT ON audit.redact, audit.reason_policy, audit.exempt,
  audit.tx_seal, audit.anchor TO datum_app;

REVOKE ALL ON ALL FUNCTIONS IN SCHEMA audit FROM PUBLIC;
GRANT EXECUTE ON FUNCTION audit.require_context() TO datum_app, datum_audit_row,
  datum_audit_event, datum_migrate, datum_owner;
GRANT EXECUTE ON FUNCTION audit.row_change() TO datum_owner, datum_migrate, datum_app;
GRANT EXECUTE ON FUNCTION audit.stmt_truncate() TO datum_owner, datum_migrate, datum_app;
GRANT EXECUTE ON FUNCTION audit.log_event(text, text, text, text, text, text, jsonb)
  TO datum_app, datum_migrate, datum_owner;
GRANT EXECUTE ON FUNCTION audit.attach(regclass) TO datum_owner, datum_migrate;
GRANT EXECUTE ON FUNCTION audit.attach_new_tables() TO datum_owner, datum_migrate;
GRANT EXECUTE ON FUNCTION audit.protect() TO datum_owner, datum_migrate;
GRANT EXECUTE ON FUNCTION audit.ensure_partitions(integer) TO datum_owner, datum_migrate;
GRANT EXECUTE ON FUNCTION audit.grant_event_insert(regclass) TO datum_owner, datum_migrate;
GRANT EXECUTE ON FUNCTION audit.scrub(oid, jsonb) TO datum_audit_row, datum_owner, datum_migrate;
GRANT EXECUTE ON FUNCTION audit.pk_of(text[], jsonb) TO datum_audit_row, datum_owner, datum_migrate;
GRANT EXECUTE ON FUNCTION audit.reason_required(oid, text) TO datum_audit_row, datum_owner, datum_migrate;
GRANT EXECUTE ON FUNCTION audit.ts_canon(timestamptz) TO datum_app, datum_owner, datum_migrate, datum_audit_row;
GRANT EXECUTE ON FUNCTION audit.canon_jsonb(jsonb) TO datum_app, datum_owner, datum_migrate, datum_audit_row;
GRANT EXECUTE ON FUNCTION audit.canon_row(audit.event) TO datum_app, datum_owner, datum_migrate, datum_audit_row;
GRANT EXECUTE ON FUNCTION audit.seal_current_tx() TO datum_owner, datum_migrate, datum_audit_row, datum_audit_event;
GRANT EXECUTE ON FUNCTION audit.verify(bigint, bigint) TO datum_app, datum_migrate, datum_owner;
GRANT EXECUTE ON FUNCTION audit.head() TO datum_app, datum_migrate, datum_owner;
GRANT EXECUTE ON FUNCTION audit.record_anchor(bigint, bytea, text, text)
  TO datum_app, datum_migrate, datum_owner;
GRANT EXECUTE ON FUNCTION audit.reseal_from(bigint) TO datum_migrate, datum_owner;
GRANT EXECUTE ON FUNCTION audit.refuse_mutation() TO datum_owner, datum_migrate;
GRANT EXECUTE ON FUNCTION audit.guc_uuid(text) TO datum_app, datum_audit_row,
  datum_audit_event, datum_migrate, datum_owner;
GRANT EXECUTE ON FUNCTION audit.guc_inet(text) TO datum_app, datum_audit_row,
  datum_audit_event, datum_migrate, datum_owner;

ALTER FUNCTION audit.require_context() OWNER TO datum_owner;
ALTER FUNCTION audit.attach(regclass) OWNER TO datum_owner;
ALTER FUNCTION audit.attach_new_tables() OWNER TO datum_owner;
ALTER FUNCTION audit.protect() OWNER TO datum_owner;
ALTER FUNCTION audit.ensure_partitions(integer) OWNER TO datum_owner;
ALTER FUNCTION audit.grant_event_insert(regclass) OWNER TO datum_owner;
ALTER FUNCTION audit.seal_current_tx() OWNER TO datum_owner;
ALTER FUNCTION audit.verify(bigint, bigint) OWNER TO datum_owner;
ALTER FUNCTION audit.head() OWNER TO datum_owner;
ALTER FUNCTION audit.record_anchor(bigint, bytea, text, text) OWNER TO datum_owner;
ALTER FUNCTION audit.reseal_from(bigint) OWNER TO datum_owner;
ALTER FUNCTION audit.refuse_mutation() OWNER TO datum_owner;
ALTER FUNCTION audit.ts_canon(timestamptz) OWNER TO datum_owner;
ALTER FUNCTION audit.canon_jsonb(jsonb) OWNER TO datum_owner;
ALTER FUNCTION audit.canon_row(audit.event) OWNER TO datum_owner;
ALTER FUNCTION audit.reason_required(oid, text) OWNER TO datum_owner;
ALTER FUNCTION audit.scrub(oid, jsonb) OWNER TO datum_owner;
ALTER FUNCTION audit.pk_of(text[], jsonb) OWNER TO datum_owner;
ALTER FUNCTION audit.guc_uuid(text) OWNER TO datum_owner;
ALTER FUNCTION audit.guc_inet(text) OWNER TO datum_owner;
