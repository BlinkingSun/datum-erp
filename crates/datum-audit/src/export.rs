...
pub struct Manifest {
    ...
    /// Datum crate version that produced the bundle.
    pub datum_version: String,
    /// PostgreSQL `server_version`.
    pub postgresql_version: String,
    /// Chain algorithm id (`datum-audit-1`).
...
/// `dictionary.md`. `report.pdf` is a documented hook for `datum-print`
...
pub async fn bundle(pool: &PgPool, selector: &Selector, dir: impl AsRef<Path>) -> Result<Manifest> {
    ...
    let files = serde_json::json!({
        ...
        "report.pdf": {
            "absent": true,
            "hook": "datum-print Wave 2b; the print primitive renders PDF/A from this bundle"
        }
    });
    let manifest = Manifest {
        export_id,
        export_schema_version: 1,
        datum_version: env!("CARGO_PKG_VERSION").to_string(),
        postgresql_version: pg_version.0,
        chain_algo: "datum-audit-1".to_string(),
        predicate,
...
pub const DICTIONARY: &str = r#"# Datum audit export dictionary (datum-audit-1)

This bundle is readable with no Datum installation. Events are one JSON object
...
`datum-print` (Wave 2b); this exporter leaves that member absent.
...
row has both set. `datum_app` holds SELECT only. Row-change columns are written
only by `audit.row_change` / `audit.stmt_truncate` (owner `datum_audit_row`).
Kernel events go through `audit.log_event` (owner `datum_audit_event`), which
cannot write `op` / `old_row` / `new_row` / `table_name`.

## Canonical row (`chain_algo = datum-audit-1`)
