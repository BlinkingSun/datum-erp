...
async fn log_event_cannot_forge_row_change() {
    ...
    let can_op: bool = sqlx::query_scalar(
        "SELECT has_column_privilege('datum_audit_event', 'audit.event', 'op', 'INSERT')",
    )
    .fetch_one(db.migrate_pool())
    .await
    .expect("col priv");
    assert!(!can_op, "datum_audit_event must not INSERT op");

    let boot = bootstrap_pool(db.database()).await;
    {
        let mut conn = boot.acquire().await.expect("boot conn");
        sqlx::query("SET ROLE datum_audit_event")
            .execute(&mut *conn)
            .await
            .expect("set role");
        let err = sqlx::query(
...
async fn tamper_breaks_verify_at_seq() {
    ...
    let head = datum_audit::head(db.app_pool())
        .await
        .expect("head")
        .expect("sealed");
    tamper_action(db.database(), "probe").await;

    let bad = datum_audit::verify(db.app_pool(), 1, head.seq)
        .await
        .expect("verify");
...
async fn reseal_diverges_from_anchor() {
    ...
    let new_head = datum_audit::head(db.app_pool())
        .await
        .expect("head2")
        .expect("sealed");
...
async fn aborted_tx_leaves_no_seal_and_same_head() {
    ...
    sqlx::query("CREATE TABLE app.probe (id uuid PRIMARY KEY, n int NOT NULL)")
        ...
        .expect("create");
    let write = write_pool(&db).await;
    let before = datum_audit::head(db.app_pool()).await.expect("head");
    ...
    let after = datum_audit::head(db.app_pool()).await.expect("head after");
    match (before, after) {
        (None, None) => {}
        (Some(a), Some(b)) => {
...
async fn export_bundle_is_self_describing() {
    ...
    let a = std::env::temp_dir().join(format!("datum-audit-export-a-{doc}"));
    let b = std::env::temp_dir().join(format!("datum-audit-export-b-{doc}"));
    let _ = fs::remove_dir_all(&a);
    let _ = fs::remove_dir_all(&b);
    let ma = datum_audit::export::bundle(db.app_pool(), &selector, &a)
        .await
        .expect("bundle a");
    let mb = datum_audit::export::bundle(db.app_pool(), &selector, &b)
        .await
        .expect("bundle b");
...
async fn require_context_refuses_foreign_txid() {
    ...
    sqlx::query("SELECT pg_catalog.set_config('datum.actor_id', $1, true)")
        .bind(&actor)
        .execute(&mut *tx)
        .await
        .expect("actor");
    sqlx::query("SELECT pg_catalog.set_config('datum.txid', '1', true)")
        .execute(&mut *tx)
        .await
        .expect("txid");
    sqlx::query("SELECT pg_catalog.set_config('datum.action', 'x', true)")
        .execute(&mut *tx)
        .await
        .expect("action");
    sqlx::query("SELECT pg_catalog.set_config('datum.source_kind', 'ui', true)")
        .execute(&mut *tx)
        .await
        .expect("source");
    let err = sqlx::query("SELECT audit.require_context()")
...
async fn require_context_refuses_missing_action() {
    ...
    sqlx::query("SELECT pg_catalog.set_config('datum.actor_id', $1, true)")
        ...
        .expect("actor");
    sqlx::query(
        "SELECT pg_catalog.set_config('datum.txid', pg_catalog.pg_current_xact_id()::text, true)",
    )
    .execute(&mut *tx)
    .await
    .expect("txid");
...
async fn truncate_is_recorded_when_owner_truncates() {
    ...
    sqlx::query("CREATE TABLE app.trunc_me (id uuid PRIMARY KEY, n int NOT NULL)")
        ...
        .await
        .expect("create");
    let write = datum_db::WritePool::new(db.migrate_pool().clone());
    let ctx = test_ctx();
    let mut tx = Tx::begin(&write, &ctx).await.expect("begin");
...
async fn migrate_login_cannot_drop_or_disable_audit_trigger() {
    ...
    let who: String = sqlx::query_scalar("SELECT session_user")
        ...
        .await
        .expect("session_user");
    assert_eq!(who, "datum_migrate");
    let is_super: bool =
        sqlx::query_scalar("SELECT rolsuper FROM pg_roles WHERE rolname = session_user")
            .fetch_one(migrate)
            .await
            .expect("rolsuper");
    assert!(!is_super, "datum_migrate must not be a superuser");
