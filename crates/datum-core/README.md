...
- `UnitCatalog` / `UnitConverter` — unit master and within-dimension convert (implemented by `datum-uom`)
...
- `SignatureError` / `SignatureGate` — verify-only gate; minting is `datum-esign`
...
`datum-esign` in Wave 2b; core ships `NoSignatures`. `UnitConverter` is
implemented by `datum-uom`. `PostingSink` is implemented by `datum-ledger`
(`GroupBuilder`).
