...
/// `ALTER TABLE ... OWNER TO datum_owner` requires CREATE on the schema.
        ...
        BEGIN
          FOREACH n IN ARRAY ARRAY['app', 'transient', 'audit'] LOOP
            IF EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = n) THEN
              EXECUTE format('GRANT USAGE, CREATE ON SCHEMA %I TO datum_owner', n);
            END IF;
          END LOOP;
        END
        $g$;
    ...
    sqlx::raw_sql(
        ...
        BEGIN
          IF EXISTS (
            ...
            WHERE n.nspname = 'datum' AND o.rolcanlogin
          ) THEN
            ALTER SCHEMA datum OWNER TO datum_owner;
          END IF;
          FOR r IN
            ...
            WHERE n.nspname IN ('datum', 'app', 'transient', 'audit')
              AND c.relkind IN ('r', 'p', 'v', 'm')
              AND o.rolcanlogin
              AND c.relname IS DISTINCT FROM '_sqlx_migrations'
          LOOP
            -- Table OWNER TO also reassigns the owned serial/identity sequence.
            EXECUTE format('ALTER TABLE %I.%I OWNER TO datum_owner', r.nspname, r.relname);
          END LOOP;
        END
        $owner$;
        "#,
...
async fn recorded(conn: &mut PgConnection, crate_name: &str, version: i64) -> Result<bool> {
    let row = sqlx::query_as::<_, (bool,)>(
        r#"
        SELECT EXISTS (
            SELECT 1 FROM datum.schema_history
...
async fn flush_pending(
    ...
    for (crate_name, version) in pending.drain(..) {
        sqlx::query(
            r#"
            INSERT INTO datum.schema_history (crate, version, applied_at, app_version)
            VALUES ($1, $2, now(), $3)
...
async fn schema_history_exists(conn: &mut PgConnection) -> Result<bool> {
    let (exists,): (bool,) =
        sqlx::query_as("SELECT to_regclass('datum.schema_history') IS NOT NULL")
            .fetch_one(&mut *conn)
            .await?;
    Ok(exists)
}
