...
-- No new tables. No datum.* GUC writes (lint-sql rule (d); 0001 only).
...
END
$fn$;
ALTER FUNCTION esign.live_instance_version(text, uuid) OWNER TO datum_owner;
...
END
$fn$;
ALTER FUNCTION esign.close_sessions_on_deactivate() OWNER TO datum_owner;
GRANT EXECUTE ON FUNCTION esign.close_sessions_on_deactivate()
  TO datum_app, datum_migrate, datum_owner;

-- identity.principal is owned by datum_owner; datum_migrate inherits that role.
SET ROLE datum_owner;
