...
use datum_identity::rbac::has_permission;
use datum_identity::{Principal, PrincipalStatus, reauth_signing, verify_login_secret};
...
pub async fn mint(tx: &mut datum_db::Tx<'_>, req: &MintRequest) -> Result<Signature> {
...
pub async fn log_refusal(
    pool: &datum_db::WritePool,
    actor: Actor,
    action: &str,
    reason: &str,
    detail: Value,
) -> Result<Uuid> {
    let mut ctx = datum_db::WriteContext::new(actor, "esign.refuse", "api");
    ctx.reason = Some(reason.to_owned());
    let mut tx = datum_db::Tx::begin(pool, &ctx).await?;
    let row: (Uuid,) = tx
        .fetch_one(
            sql_query_as(
