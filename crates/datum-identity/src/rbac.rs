...
pub async fn seed_bundles(
    ...
    for bundle in bundles {
        ...
        let row: (Uuid,) = tx
            ...
            .await
            .map_err(map_tx)?;
        let role = RoleId(datum_core::Identifier::from_uuid(row.0));
...
pub async fn assign_role(tx: &mut datum_db::Tx<'_>, principal: UserId, role: RoleId) -> Result<()> {
    tx.execute(
        sql_query(
...
pub async fn has_permission(tx: &mut datum_db::Tx<'_>, actor: Actor, key: &str) -> Result<bool> {
    let row: (bool,) = tx
        .fetch_one(
            sql_query_as(
                r#"SELECT EXISTS (
