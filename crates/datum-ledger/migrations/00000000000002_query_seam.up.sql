...
-- datum_app already has SELECT on ledger.posting; audit.require_context()
...
END
$fn$;
ALTER FUNCTION ledger.has_postings(uuid) OWNER TO datum_owner;
REVOKE ALL ON FUNCTION ledger.has_postings(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION ledger.has_postings(uuid) TO datum_app;
...
END
$fn$;
ALTER FUNCTION ledger.has_quantity_at(uuid) OWNER TO datum_owner;
REVOKE ALL ON FUNCTION ledger.has_quantity_at(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION ledger.has_quantity_at(uuid) TO datum_app;
