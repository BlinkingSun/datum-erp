...
/// Convert `entered` through [`datum_uom::to_stock`] and, if a residual remains,
...
async fn residual_of(
    ...
    match entered.dimension {
        DimensionKind::Count => {
            pack(datum_uom::to_stock::<CountDim>(tx, catalog, item, entered, ctx).await?)
        }
        DimensionKind::Length => {
            pack(datum_uom::to_stock::<LengthDim>(tx, catalog, item, entered, ctx).await?)
        }
        DimensionKind::Mass => {
            pack(datum_uom::to_stock::<MassDim>(tx, catalog, item, entered, ctx).await?)
        }
        DimensionKind::Time => {
            pack(datum_uom::to_stock::<TimeDim>(tx, catalog, item, entered, ctx).await?)
        }
        DimensionKind::Volume => {
            pack(datum_uom::to_stock::<VolumeDim>(tx, catalog, item, entered, ctx).await?)
        }
        DimensionKind::Area => {
            pack(datum_uom::to_stock::<AreaDim>(tx, catalog, item, entered, ctx).await?)
        }
        _ => Err(Error::from_posting(datum_core::PostingError::Shape(
            "unknown dimension".into(),
        ))),
    }
}

fn pack<D: datum_core::Dimension>(conv: datum_uom::StockConversion<D>) -> Result<AnyQuantity> {
