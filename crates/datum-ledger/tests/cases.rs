...
use datum_db::Tx;
use datum_ledger::{GroupBuilder, UOM_CONVERSION_RESIDUAL, post, post_uom_residual_flush, reverse};
...
async fn case_a_receive_into_quarantine() {
    let db = datum_test::db_case!("case_a");
...
async fn case_b_release_quarantine() {
    let db = datum_test::db_case!("case_b");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn case_c_issue_to_wo() {
    let db = datum_test::db_case!("case_c");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn case_d_complete_screws() {
    let db = datum_test::db_case!("case_d");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn case_e_scrap_at_op() {
    let db = datum_test::db_case!("case_e");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn case_f_cycle_count_short() {
    let db = datum_test::db_case!("case_f");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn case_g_ship_to_customer() {
    let db = datum_test::db_case!("case_g");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn case_h_customer_return() {
    let db = datum_test::db_case!("case_h");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn case_i_rework_recovery() {
    let db = datum_test::db_case!("case_i");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn case_j_outside_processing() {
    let db = datum_test::db_case!("case_j");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn case_k_inch_issue_and_dust() {
    let db = datum_test::db_case!("case_k");
    common::migrate(&db).await;
    let pool = write_pool(&db);
    ...
    let leftover = datum_ledger::post_uom_conversion_residual(
        &mut tx,
        w.bar,
        w.available,
        w.rounding,
...
async fn case_l_reverse_c() {
    let db = datum_test::db_case!("case_l");
    common::migrate(&db).await;
    let pool = write_pool(&db);
