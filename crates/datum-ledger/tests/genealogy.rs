...
use datum_db::Tx;
use datum_ledger::{TraceStart, trace_backward, trace_forward};

use common::*;

#[tokio::test]
async fn trace_backward_and_forward_follow_consumption() {
    let db = datum_test::db_case!("geneal");
    ...
    let back = trace_backward(
        &mut tx,
        TraceStart::Posting(datum_core::PostingId(consuming)),
    )
    .await
    .expect("backward");
    assert!(
    ...
    let fwd = trace_forward(
        &mut tx,
        TraceStart::Posting(datum_core::PostingId(consumed)),
    )
    .await
    .expect("forward");
    assert!(
