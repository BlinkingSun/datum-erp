...
use datum_core::{Boundary, GroupKind, PostingIntent, PostingSink, ValueAccount};
use datum_db::Tx;
use datum_ledger::{
...
async fn reverse_migration_tested() {
    let db = datum_test::db_case!("mig_rev");
    ...
    let n2: i64 = sqlx::query_scalar("SELECT count(*) FROM pg_namespace WHERE nspname = 'ledger'")
        ...
        .unwrap();
    assert_eq!(n2, 0);
    sqlx::query("DELETE FROM datum.schema_history WHERE crate = 'datum-ledger'")
        .execute(db.migrate_pool())
        .await
...
async fn enum_bijection_round_trip() {
    let db = datum_test::db_case!("enums");
    common::migrate(&db).await;
...
async fn canary_ledger_constraints_are_armed() {
    let db = datum_test::db_case!("canary_led");
    common::migrate(&db).await;
...
async fn zl000_group_has_no_header() {
    let db = datum_test::db_case!("zl000");
    common::migrate(&db).await;
    let pool = write_pool(&db);
    ...
    .bind(datum_core::Identifier::generate().as_uuid())
    .bind(w.bar.as_uuid())
    .bind(FT.0)
...
async fn zl_via_raw(name: &str, sqlstate: &str, build: impl FnOnce(&mut GroupBuilder, &World)) {
    let db = datum_test::db_case!(name);
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn p0_like() {
    let db = datum_test::db_case!("zl001");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn zl004_cost_element_reclassified() {
    zl_via_raw("zl004", "ZL004", |b, w| {
        ...
        .unwrap();
        b.contribute(PostingIntent::Value(datum_core::ValuePosting {
            account: ValueAccount::ApAccrual,
            cost_element: datum_core::CostElement::Outside,
            cost_object: None,
            amount: usd("-2.36"),
            values: None,
        }))
...
async fn zl005_allocation_incomplete() {
    let db = datum_test::db_case!("zl005");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn zl006_reversal_not_exact() {
    let db = datum_test::db_case!("zl006");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn zl007_layers_not_restored() {
    let db = datum_test::db_case!("zl007");
    common::migrate(&db).await;
    let pool = write_pool(&db);
    ...
    datum_ledger::reverse(&mut tx, c, "undo")
        .await
...
async fn group_actor_matches_audit() {
    let db = datum_test::db_case!("actor_eq");
    common::migrate(&db).await;
    let pool = write_pool(&db);
    let act = actor();
...
async fn empty_group_and_unfinalized() {
    let db = datum_test::db_case!("empty");
    common::migrate(&db).await;
    let pool = write_pool(&db);
    ...
    assert!(matches!(
        err,
        datum_ledger::Error::Posting(datum_core::PostingError::EmptyGroup)
    ));
    let mut b2 = GroupBuilder::new(GroupKind::Movement, movement_header("drop"));
    b2.contribute(PostingIntent::Quantity(q_post(
        datum_core::ItemId::generate(),
        qty_ft("1.0000"),
        datum_core::LocationId::generate(),
        None,
        None,
    )))
    .ok();
    datum_ledger::bind_tx(&mut b2, &mut tx).await.unwrap();
    drop(b2);
    let err = datum_ledger::commit(tx, &[]).await.expect_err("poison");
...
async fn lineage_required_and_after_finalize() {
    let db = datum_test::db_case!("lineage");
    common::migrate(&db).await;
    let pool = write_pool(&db);
    ...
    assert!(
        matches!(
            err,
            datum_ledger::Error::Posting(datum_core::PostingError::LineageRequired(_))
        ),
    ...
    let after = b2
        ...
        .unwrap_err();
    assert!(matches!(after, datum_core::PostingError::AfterFinalize));
    db.finish().await.unwrap();
}
