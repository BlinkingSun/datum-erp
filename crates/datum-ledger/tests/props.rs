...
async fn p2b_cost_element() {
    ...
    b.contribute(PostingIntent::Value(datum_core::ValuePosting {
        ...
        cost_element: datum_core::CostElement::Labor,
        cost_object: None,
        amount: usd("-23.60"),
        values: None,
    }))
...
async fn boundary_matrix() {
    let db = datum_test::db_case!("bmat");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn scale_exactness() {
    let db = datum_test::db_case!("scale");
    common::migrate(&db).await;
    let pool = write_pool(&db);
    ...
    assert!(
        code == "23514"
            ...
            || matches!(err, datum_ledger::Error::Db(_)),
...
async fn dust_bounded() {
    let db = datum_test::db_case!("dust");
    common::migrate(&db).await;
    let pool = write_pool(&db);
    ...
    let code = pg_code_ledger(&err);
    assert!(
        code == "23514" || matches!(err, datum_ledger::Error::Db(_)),
