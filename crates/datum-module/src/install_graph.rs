...
/// `datum-mod-items` embedded migrator (path relative to this crate's manifest).
pub static ITEMS_MIGRATOR: Migrator = sqlx::migrate!("../../modules/items/migrations");
/// `datum-mod-locations` embedded migrator.
...
pub struct ModuleInstallSpec {
    ...
    pub id: &'static str,
    /// `datum.schema_history` crate label.
...
pub async fn migrate_wave_2s1_modules(pool: &datum_db::Pool) -> Result<()> {
