...
use datum_module::{Kernel, ModuleManifest, Profile};
...
async fn machine_registered_through_kernel_is_transitionable_after_build() {
    ...
    assert!(
        matches!(err, datum_statemachine::Error::Frozen),
        "got {err:?}"
    );
        ...
        if expect_required {
            assert!(
                kernel.profile.required_edges().iter().any(|e| matches!(
                    e,
                    datum_module::SignatureEdge::Required { edge: name, .. } if name == edge
                )),
...
async fn hook_postings_reach_the_ledger() {
    ...
    .expect("contribute");
    drop(sink);
    let err = datum_ledger::commit(tx, &[])
        .await
        .expect_err("unfinalized poisons");
    assert!(
        matches!(err, datum_ledger::Error::Unfinalized),
        "got {err:?}"
    );
    db.finish().await.expect("finish");
}

fn common_boot() -> datum_db::WriteContext {
    let mut ctx = datum_db::WriteContext::new(
        datum_core::Actor {
...
"#;
    ...
    assert!(
        kernel
            .subscriptions
            .iter()
            .any(|s| s.event == "inventory.lot_received" && s.subscriber == "datum-jobs")
    );
    assert!(
        kernel
            .subscriptions
