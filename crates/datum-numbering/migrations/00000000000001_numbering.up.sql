...
-- Reversible. Tables owned by datum_owner.

CREATE SCHEMA IF NOT EXISTS numbering AUTHORIZATION datum_owner;

REVOKE ALL ON SCHEMA numbering FROM PUBLIC;
GRANT USAGE ON SCHEMA numbering TO datum_app, datum_migrate, datum_owner;

ALTER DEFAULT PRIVILEGES FOR ROLE datum_owner IN SCHEMA numbering
  GRANT SELECT, INSERT, UPDATE ON TABLES TO datum_app;
ALTER DEFAULT PRIVILEGES FOR ROLE datum_migrate IN SCHEMA numbering
  GRANT SELECT, INSERT, UPDATE ON TABLES TO datum_app;

INSERT INTO datum.schema_class (nspname, class) VALUES ('numbering', 'app')
...
BEGIN
  IF to_regclass('audit.exempt') IS NULL THEN
    RAISE EXCEPTION 'numbering: audit.exempt is missing; migrate datum-audit first';
  END IF;
...
END
$fn$;
ALTER FUNCTION numbering.register_exempt() OWNER TO datum_owner;
REVOKE ALL ON FUNCTION numbering.register_exempt() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION numbering.register_exempt() TO datum_migrate, datum_owner;
...
ALTER TABLE numbering.counter OWNER TO datum_owner;
...
GRANT SELECT, INSERT, UPDATE ON numbering.counter TO datum_app;
...
END
$fn$;
ALTER FUNCTION numbering.bind_exempt() OWNER TO datum_owner;
REVOKE ALL ON FUNCTION numbering.bind_exempt() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION numbering.bind_exempt() TO datum_migrate, datum_owner;
...
END
$fn$;
ALTER FUNCTION numbering.render(text, bigint, timestamptz) OWNER TO datum_owner;
...
END
$fn$;
ALTER FUNCTION numbering.define(text, text, text) OWNER TO datum_owner;
...
END
$fn$;
ALTER FUNCTION numbering.next_number(text, text) OWNER TO datum_owner;
...
END
$fn$;
ALTER FUNCTION numbering.server_now() OWNER TO datum_owner;
