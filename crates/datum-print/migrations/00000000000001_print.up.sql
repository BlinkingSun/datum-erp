...
SELECT
  ...
  pg_catalog.set_config('datum.action',        'print.migrate', true),
  pg_catalog.set_config('datum.source_kind',   'migration', true);

CREATE SCHEMA IF NOT EXISTS print AUTHORIZATION datum_migrate;

REVOKE ALL ON SCHEMA print FROM PUBLIC;
GRANT USAGE ON SCHEMA print TO datum_app;
GRANT USAGE, CREATE ON SCHEMA print TO datum_migrate, datum_owner;

INSERT INTO datum.schema_class (nspname, class) VALUES ('print', 'app')
ON CONFLICT (nspname) DO UPDATE SET class = EXCLUDED.class;

ALTER DEFAULT PRIVILEGES FOR ROLE datum_migrate IN SCHEMA print
  GRANT SELECT, INSERT, UPDATE ON TABLES TO datum_app;
ALTER DEFAULT PRIVILEGES FOR ROLE datum_migrate IN SCHEMA print
  GRANT TRIGGER ON TABLES TO datum_owner;
ALTER DEFAULT PRIVILEGES FOR ROLE datum_owner IN SCHEMA print
  GRANT SELECT, INSERT, UPDATE ON TABLES TO datum_app;

-- Boot stamp: composition root writes the installation profile id here.
-- `datum.config_version` is spec_version (invariant 17), not the profile id.
...
ALTER TABLE print.install OWNER TO datum_owner;
...
ALTER TABLE print.template OWNER TO datum_owner;
...
ALTER TABLE print.render_log OWNER TO datum_owner;
...
ALTER SCHEMA print OWNER TO datum_owner;
