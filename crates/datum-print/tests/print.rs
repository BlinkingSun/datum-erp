...
use datum_db::Tx;
use datum_documents::{BlobStore, Manifest, attach, create, new_revision};
use datum_esign::{MintRequest, SessionPolicy, mint};
use datum_identity::{
...
async fn mint_for_record(write: &datum_db::WritePool, rec: RecordRef, body: serde_json::Value) {
    ...
    let roles = seed_bundles(
        &mut tx,
        &[datum_identity::rbac::RoleBundle {
    ...
    let req = MintRequest {
        ...
        meaning: datum_core::SignatureMeaning("Approved".into()),
        reason: None,
        record: rec.clone(),
        doc_type: "test.doc".into(),
        projection: body,
        instance: datum_esign::InstanceTriple {
            doc_type: "test.doc".into(),
            doc_id: rec.id,
            state: "Effective".into(),
            version: rec.version,
        },
        permission: datum_core::PermissionKey("documents.approve".into()),
...
async fn manifestation_block_inline_regulated() {
    ...
    let mut tx = Tx::begin(&write, &write_ctx("print.render")).await.unwrap();
    assert_eq!(
        tx.setting("datum.config_version").await.unwrap(),
        SPEC_VERSION
    );
    let out = render(
        &mut tx,
...
async fn writes_go_through_tx() {
    let db = datum_test::db_case!("print_writes_tx");
    migrate(&db, "plain-shop").await;
    let err = query(
