...
//! HTTP handlers for `datum-customfields` published APIs.
...
pub fn envelope_arm(
    ...
    match e {
        datum_customfields::Error::NotFound => (
            "NOT_FOUND",
            axum::http::StatusCode::NOT_FOUND,
            None,
            e.to_string(),
        ),
        datum_customfields::Error::StateMachine(sm) => match sm {
        ...
        datum_customfields::Error::UnknownValidationRule { .. }
        | datum_customfields::Error::ValidationFailed { .. }
        | datum_customfields::Error::RequiredFieldMissing { .. }
        | datum_customfields::Error::TypeMismatch { .. }
        | datum_customfields::Error::TypeChangeRefused { .. }
        | datum_customfields::Error::AuditRefused
        | datum_customfields::Error::UnknownProfile(_) => (
            "VALIDATION",
            axum::http::StatusCode::BAD_REQUEST,
            None,
            e.to_string(),
        ),
        datum_customfields::Error::RetireForbidden { .. } => (
            "FORBIDDEN",
            axum::http::StatusCode::FORBIDDEN,
            None,
            e.to_string(),
        ),
        datum_customfields::Error::Retired | datum_customfields::Error::AlreadyRetired => (
            "CONFLICT",
...
fn definition_engine(profile: &str) -> Result<Engine> {
    ...
    engine.register_machine(datum_customfields::definition_machine(profile)?)?;
    engine.freeze()?;
    Ok(engine)
}
...
async fn merge_item_fields(tx: &mut Tx<'_>, item: ItemId) -> Result<Vec<ValueWire>> {
    ...
    let defs = datum_customfields::definitions_for(tx, entity).await?;
    let mut out = Vec::with_capacity(defs.len());
    for def in defs {
        let value = datum_customfields::get(tx, entity, record, &def.key).await?;
...
async fn define_inner(
    ...
    let id = datum_customfields::define(&mut tx, spec).await?;
    let payload = json!({
...
async fn definitions_for_inner(
    ...
    .await?;
    let defs = datum_customfields::definitions_for(&mut tx, entity).await;
    tx.rollback().await?;
    let defs = defs?;
    Ok(json!({
        "data": defs,
...
async fn retire_inner(
    ...
    let ctx = datum_customfields::retire_context(ctx, def_id);
    let write = state.write_pool();
    let mut tx = Tx::begin(&write, &ctx).await?;
    if let Some(replay) = idempotency::replay(&mut tx, key, &hash).await? {
        tx.commit().await?;
    ...
    datum_customfields::retire(&mut tx, &engine, def_id, &ctx).await?;
    let payload = json!({
...
async fn set_inner(
    ...
    datum_mod_items::get(state.pool(), item_id).await?;
    let ctx = write_context(
        &session,
        "customfields.set",
        request_id,
    ...
    for field in body.fields {
        ...
        datum_customfields::set(&mut tx, "items.item", record, &field.key, value).await?;
    }
