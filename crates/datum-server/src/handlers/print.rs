...
//! HTTP transport for `datum-print`. Published crate APIs only (R-2s-3).
...
/// Map `datum_print::Error` into the docs/10 envelope.
pub fn envelope_arm(
    e: &datum_print::Error,
) -> (&'static str, axum::http::StatusCode, Option<&str>, String) {
    match e {
        datum_print::Error::NotFound => (
            "NOT_FOUND",
            axum::http::StatusCode::NOT_FOUND,
            None,
            e.to_string(),
        ),
        datum_print::Error::UnknownTemplate(_) => (
            "NOT_FOUND",
            axum::http::StatusCode::NOT_FOUND,
            Some("template_id"),
            e.to_string(),
        ),
        datum_print::Error::UnsupportedFormat => (
            "VALIDATION",
            axum::http::StatusCode::BAD_REQUEST,
            Some("format"),
            e.to_string(),
        ),
        datum_print::Error::UnknownProfile(_) => (
            "VALIDATION",
            axum::http::StatusCode::BAD_REQUEST,
            Some("profile"),
            e.to_string(),
        ),
        datum_print::Error::Documents(datum_documents::Error::NotFound) => (
            "NOT_FOUND",
            axum::http::StatusCode::NOT_FOUND,
            None,
            e.to_string(),
...
async fn list_templates_inner(
    ...
    headers: &H,
    request_id: &str,
) -> Result<ListBody<datum_print::TemplateSummary>> {
    let _session =
...
async fn archive_inner(
    ...
    let format = Format::parse(&row.output_format).ok_or(datum_print::Error::UnsupportedFormat)?;
