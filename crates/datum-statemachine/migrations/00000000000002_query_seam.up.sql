...
-- SECURITY DEFINER — R-2s-8 does not exempt datum-statemachine). datum_app
...
$$;
ALTER FUNCTION sm.current_state(text, uuid) OWNER TO datum_owner;
REVOKE ALL ON FUNCTION sm.current_state(text, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION sm.current_state(text, uuid) TO datum_app;
...
$$;
ALTER FUNCTION sm.instance_exists(text, uuid) OWNER TO datum_owner;
REVOKE ALL ON FUNCTION sm.instance_exists(text, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION sm.instance_exists(text, uuid) TO datum_app;
...
$$;
ALTER FUNCTION sm.machine_id_for(text) OWNER TO datum_owner;
REVOKE ALL ON FUNCTION sm.machine_id_for(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION sm.machine_id_for(text) TO datum_app;
