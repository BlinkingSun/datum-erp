//! Engine write seam: datum_app cannot DML sm.instance; transitions still
...
use datum_db::{Error as DbError, Tx, WritePool};
use datum_statemachine::{DocRef, Engine, current_state};
...
async fn direct_update_instance_as_app_is_42501() {
    ...
    assert!(
        table_priv(db.app_pool(), "datum_app", "SELECT").await,
        "SELECT on sm.instance stays"
    );
    assert!(
        !table_priv(db.app_pool(), "datum_app", "UPDATE").await,
        "UPDATE revoked from datum_app"
    );
    assert!(
        !table_priv(db.app_pool(), "datum_app", "INSERT").await,
        "INSERT revoked from datum_app"
    );
    assert!(
        !table_priv(db.app_pool(), "datum_app", "DELETE").await,
        "DELETE revoked from datum_app"
    );
    ...
    assert!(
        ...
        "direct UPDATE sm.instance as datum_app must be 42501, got {err:?}"
    );
...
async fn engine_transition_succeeds_for_app_and_migrate() {
    ...
    for (label, write) in [
        ("datum_app", write_pool(&db)),
        ("datum_migrate", write_pool_migrate(&db)),
    ] {
        let doc = DocRef {
            doc_type: "wo".into(),
            doc_id: Identifier::generate(),
