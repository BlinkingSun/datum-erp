...
async fn migrate_down_then_up() {
    ...
    wicket_audit::uninstall_privileged(&boot)
        .await
        .expect("uninstall");
    migrator
...
async fn grants_match_d3_section_1_3() {
    ...
    assert!(priv_ok(pool, "wicket_app", "audit.event", "SELECT").await);
    assert!(!priv_ok(pool, "wicket_app", "audit.event", "INSERT").await);
    assert!(!priv_ok(pool, "wicket_app", "audit.event", "UPDATE").await);
    assert!(!priv_ok(pool, "wicket_app", "audit.event", "DELETE").await);
    assert!(!priv_ok(pool, "wicket_app", "audit.event", "TRUNCATE").await);

    assert!(priv_ok(pool, "wicket_audit_row", "audit.event", "INSERT").await);
    assert!(!priv_ok(pool, "wicket_audit_row", "audit.event", "UPDATE").await);
    assert!(!priv_ok(pool, "wicket_audit_row", "audit.event", "DELETE").await);

    assert!(!priv_ok(pool, "wicket_audit_event", "audit.event", "UPDATE").await);
    assert!(!priv_ok(pool, "wicket_audit_event", "audit.event", "DELETE").await);
    ...
    for rel in &event_rels {
            ...
            SELECT EXISTS (
              SELECT 1
               ...
               WHERE c.oid = $1::regclass
                 AND r.rolname = 'wicket_audit_event'
        ...
        assert!(
            !table_level_insert,
            "wicket_audit_event must not hold table-level INSERT on {rel}"
        );

        for col in EVENT_WRITER_COLS {
            let ok: bool = sqlx::query_scalar(
                "SELECT has_column_privilege('wicket_audit_event', $1::regclass, $2, 'INSERT')",
            )
            .bind(rel)
            .bind(*col)
            .fetch_one(pool)
            .await
            .expect("col grant");
            assert!(ok, "wicket_audit_event INSERT {col} on {rel}");
        }
        for col in FORBIDDEN_EVENT_COLS {
            ...
            assert!(!ok, "wicket_audit_event must not INSERT {col} on {rel}");
        }
    }
    ...
    assert_eq!(row_owner, "wicket_audit_row");
    ...
    assert_eq!(event_owner, "wicket_audit_event");
    db.finish().await.expect("finish");
}
...
/// D-2b-12(1): skip set is `wicket.schema_class`, not the literal name
...
async fn event_trigger_skips_transient_class_by_catalog() {
    ...
    sqlx::raw_sql(AssertSqlSafe(
        r#"
        CREATE SCHEMA inventory_transient AUTHORIZATION wicket_migrate;
    ...
    sqlx::raw_sql(AssertSqlSafe(
        r#"
        CREATE SCHEMA weird_transient AUTHORIZATION wicket_migrate;
        GRANT USAGE, CREATE ON SCHEMA weird_transient TO wicket_migrate, wicket_owner;
        INSERT INTO wicket.schema_class (nspname, class)
          VALUES ('weird_transient', 'app')
          ON CONFLICT (nspname) DO UPDATE SET class = EXCLUDED.class;
        ALTER DEFAULT PRIVILEGES FOR ROLE wicket_migrate IN SCHEMA weird_transient
          GRANT TRIGGER ON TABLES TO wicket_owner;
    ...
    sqlx::raw_sql(AssertSqlSafe(
        "CREATE TABLE wicket.probe_skipped (id int PRIMARY KEY)".to_owned(),
    ))
    .execute(pool)
    .await
    .expect("wicket.probe_skipped");
    assert!(
        !has_zz_audit_row(pool, "wicket", "probe_skipped").await,
        "nspname wicket must skip even though class is app"
    );
...
/// history after `wicket.schema_history` is attached.
#[tokio::test]
async fn runner_records_history_on_an_attached_schema_history() {
    let db = db_case!("hist_attached");
    migrate_and_install(&db).await;
    wicket_audit::attach(db.migrate_pool(), "wicket.schema_history")
        .await
        .expect("attach schema_history");
    assert!(
        has_zz_audit_row(db.migrate_pool(), "wicket", "schema_history").await,
        "schema_history must carry zz_audit_row"
    );

    let later = toy(1, "later", "SELECT 1;");
    wicket_db::migrate::run(db.migrate_pool(), &[("later-crate", &later)])
        .await
        .expect("run after attach");

    let n: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM wicket.schema_history WHERE crate = 'later-crate' AND version = 1",
    )
    .fetch_one(db.migrate_pool())
    .await
    .expect("count");
