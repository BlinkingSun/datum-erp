...
//! Writes go through [`wicket_db::Tx`] only (CONTRACT §5a).
...
use wicket_statemachine::{Engine, instance_exists};
...
async fn spawn_definition(tx: &mut Tx<'_>, id: DefinitionId) -> Result<()> {
    let profile = match tx.setting("wicket.config_version").await?.as_str() {
...
pub async fn retire(
    ...
    let action = tx.setting("wicket.action").await?;
