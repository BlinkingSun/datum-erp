...
/// - any table outside `audit` / `transient` / `wicket` / app-class schemas
/// - any `DELETE` grant to `wicket_app` outside `transient`
...
async fn stray_tables(pool: &Pool, out: &mut Vec<String>) -> Result<()> {
    ...
    match sqlx::query_as::<_, (String,)>(
        "SELECT nspname::text FROM wicket.schema_class WHERE class = 'app'",
    )
    .fetch_all(pool)
    .await
    {
    ...
    for (schema, table) in rows {
        ...
        out.push(format!(
            "table {schema}.{table} outside audit/transient/wicket/app-class schemas"
        ));
    }
    Ok(())
}
...
async fn app_delete(pool: &Pool, out: &mut Vec<String>) -> Result<()> {
    ...
    for (schema, table) in rows {
        out.push(format!(
            "DELETE grant to wicket_app on {schema}.{table} outside transient"
        ));
    }
    Ok(())
}
