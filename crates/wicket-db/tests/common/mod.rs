...
LANGUAGE plpgsql AS $$
BEGIN
  IF nullif(current_setting('wicket.actor_id', true), '') IS NULL THEN
  ...
  IF nullif(current_setting('wicket.txid', true), '')::xid8
    ...
    RAISE EXCEPTION 'wicket: refused write with context from another transaction'
      USING ERRCODE = '42501';
  END IF;
  IF nullif(current_setting('wicket.action', true), '') IS NULL
     OR nullif(current_setting('wicket.source_kind', true), '') IS NULL THEN
    RAISE EXCEPTION 'wicket: refused write with no declared action'
      USING ERRCODE = '42501';
  END IF;
...
/// Cloned TestDb databases are owned by the bootstrap user, so `wicket_migrate`
...
pub async fn grant_create_on_database(database: &str) {
    ...
    // Match wicket-test: sqlx does not default a missing user the way libpq does.
