...
use wicket_db::migrate::ADVISORY_LOCK_KEY;
use wicket_db::{MIGRATOR, ddl, migrate};
...
/// sqlx's default `_sqlx_migrations` lives in `public`, which `wicket_migrate`
...
async fn catalog_wicket(pool: &sqlx::PgPool) -> String {
...
/// No `app` / `transient` / `wicket` table is owned by a login role (addendum 2).
async fn assert_no_login_owned_tables(pool: &sqlx::PgPool) {
    let rows: Vec<(String, String, String)> = sqlx::query_as(
        ...
        JOIN pg_namespace n ON n.oid = c.relnamespace
        JOIN pg_roles o ON o.oid = c.relowner
        WHERE n.nspname IN ('app', 'transient', 'wicket')
          AND c.relkind IN ('r', 'p')
          AND o.rolcanlogin
    ...
    assert!(
        rows.is_empty(),
        "tables owned by a login role (want wicket_owner): {rows:?}"
    );
}
...
async fn migrate_runs_in_order_under_lock() {
    ...
    let first = tokio::spawn(async move {
        migrate::run(&pool, &[("wicket-db", &MIGRATOR), ("toy_a", &toy_a)]).await
    });
    ...
    let order: Vec<String> = sqlx::query_scalar(
        "SELECT crate FROM wicket.schema_history ORDER BY applied_at, crate, version",
    )
    .fetch_all(db.migrate_pool())
    .await
    .expect("history");
    assert!(order.iter().any(|c| c == "wicket-db"), "history={order:?}");
    let toys: Vec<String> =
    ...
    let history_toys: Vec<(String, i64)> = sqlx::query_as(
        "SELECT crate, version FROM wicket.schema_history WHERE crate LIKE 'toy%' ORDER BY applied_at, crate",
    )
    .fetch_all(db.migrate_pool())
    .await
    .expect("toy history");
...
async fn migrate_is_idempotent() {
    ...
    common::grant_create_on_database(db.database()).await;
    migrate::run(db.migrate_pool(), &[("wicket-db", &MIGRATOR)])
        .await
        .expect("first");
    migrate::run(db.migrate_pool(), &[("wicket-db", &MIGRATOR)])
        .await
        .expect("second");
    let n: i64 =
        sqlx::query_scalar("SELECT count(*) FROM wicket.schema_history WHERE crate = 'wicket-db'")
            .fetch_one(db.migrate_pool())
            .await
            .expect("count");
    let versions: i64 = sqlx::query_scalar(
        "SELECT count(DISTINCT version) FROM wicket.schema_history WHERE crate = 'wicket-db'",
    )
    .fetch_one(db.migrate_pool())
    .await
    .expect("distinct");
...
async fn migrate_down_then_up() {
    ...
    let before = catalog_wicket(db.migrate_pool()).await;
    migrator
        .undo(db.migrate_pool(), 0)
        .await
        .expect("down to placeholder");
    let gone: bool = sqlx::query_scalar("SELECT to_regclass('wicket.schema_history') IS NULL")
        .fetch_one(db.migrate_pool())
        .await
        .expect("dropped");
    assert!(gone, "0001 down must drop schema_history");
    migrator.run(db.migrate_pool()).await.expect("up again");
    let after = catalog_wicket(db.migrate_pool()).await;
    assert_eq!(
        before, after,
...
async fn tables_not_owned_by_login_role() {
    ...
    assert!(
        !owners.is_empty(),
        "0001 must create tables in schema wicket"
    );
    for (table, role, can_login) in &owners {
        assert_eq!(role, "wicket_owner", "{table} owner={role}");
