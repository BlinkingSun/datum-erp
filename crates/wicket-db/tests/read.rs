...
use wicket_db::{Error, ReadPool};
use wicket_test::db_case;

use common::app_url;

/// Probe table `wicket_app` may SELECT and must not INSERT/UPDATE/DELETE.
async fn install_select_only_probe(db: &wicket_test::TestDb) {
    sqlx::raw_sql(
        r#"
        CREATE TABLE app.read_probe (id int PRIMARY KEY, n int NOT NULL DEFAULT 0);
        REVOKE INSERT, UPDATE, DELETE ON app.read_probe FROM wicket_app;
...
async fn read_pool_selects_under_app_and_migrate() {
    ...
    for (label, read) in [
        ("wicket_app wrap", &read_app),
        ("wicket_migrate wrap", &read_mig),
        ("wicket_app connect", &connected),
    ] {
        let (n,): (i32,) = read
        ...
        let actor: (Option<String>,) = read
            .fetch_one(sqlx::query_as(
                "SELECT pg_catalog.current_setting('wicket.actor_id', true)",
            ))
            .await
...
/// at GRANT (`42501`) as `wicket_app`. The probe row count is unchanged.
#[tokio::test]
