...
use wicket_db::{Error, Tx, WICKET_SETTINGS, WritePool, retry_serializable};
...
async fn tx_actor_bound_to_transaction_id() {
    ...
    let stored = tx.setting("wicket.txid").await.expect("txid guc");
    let live = tx.pg_txid().await.expect("pg_current_xact_id");
    assert_eq!(stored, live, "wicket.txid must equal pg_current_xact_id()");
    let actor = tx.setting("wicket.actor_id").await.expect("actor_id");
...
async fn connect_sets_utc_and_application_name() {
    ...
    let pool = wicket_db::connect(&app_url(db.database()))
        .await
        .expect("connect");
    ...
    assert_eq!(row.1, "wicket");
    pool.close().await;
    db.finish().await.expect("finish");
}

/// All eighteen `wicket.*` settings readable inside the transaction, none after commit.
#[tokio::test]
async fn tx_sets_every_setting() {
    ...
    assert_eq!(
        tx.setting("wicket.actor_id").await.unwrap(),
        ctx.actor.id.as_uuid().to_string()
    );
    assert_eq!(tx.setting("wicket.actor_kind").await.unwrap(), "User");
    assert_eq!(tx.setting("wicket.actor_display").await.unwrap(), "Ada");
    assert_eq!(tx.setting("wicket.acting_for").await.unwrap(), "for-1");
    assert_eq!(tx.setting("wicket.session_id").await.unwrap(), "sess-1");
    assert_eq!(tx.setting("wicket.request_id").await.unwrap(), "req-1");
    assert_eq!(tx.setting("wicket.source_kind").await.unwrap(), "test");
    assert_eq!(tx.setting("wicket.source_device").await.unwrap(), "dev-1");
    assert_eq!(tx.setting("wicket.source_ip").await.unwrap(), "127.0.0.1");
    assert_eq!(tx.setting("wicket.client_app").await.unwrap(), "test");
    assert_eq!(tx.setting("wicket.action").await.unwrap(), "test.begin");
    assert_eq!(tx.setting("wicket.reason").await.unwrap(), "because");
    assert_eq!(tx.setting("wicket.doc_type").await.unwrap(), "wo");
    assert_eq!(tx.setting("wicket.doc_id").await.unwrap(), "doc-1");
    assert_eq!(tx.setting("wicket.esign_id").await.unwrap(), "es-1");
    assert_eq!(tx.setting("wicket.config_version").await.unwrap(), "cfg-1");
    let app_version = tx.setting("wicket.app_version").await.unwrap();
    assert!(
        app_version.starts_with(env!("CARGO_PKG_VERSION")),
        "app_version={app_version}"
    );
    let txid = tx.setting("wicket.txid").await.unwrap();
...
/// Mid-Tx bind stamps `wicket.esign_id` for remaining statements (D-2b-1).
...
async fn bind_esign_id_stamps_mid_transaction() {
    ...
    let mut tx = Tx::begin(&write, &ctx).await.expect("begin");
    assert_eq!(
        tx.setting("wicket.esign_id").await.expect("begin guc"),
        "begin-time"
    );
    let id = wicket_core::Identifier::generate().as_uuid().to_string();
    tx.bind_esign_id(&id).await.expect("bind");
    assert_eq!(tx.setting("wicket.esign_id").await.expect("after"), id);
    assert_eq!(
    ...
    let pool = wicket_db::connect(&app_url(db.database()))
        ...
        .expect("probe pool");
    let leaked: (Option<String>,) =
        sqlx::query_as("SELECT pg_catalog.current_setting('wicket.esign_id', true)")
            .fetch_one(&pool)
            .await
            .expect("probe");
    assert!(
...
/// different actors; `current_setting('wicket.actor_id')` differs per
...
async fn pooled_connection_cannot_leak_actor() {
    ...
    let a = tx.setting("wicket.actor_id").await.expect("a");
    ...
    let b = tx.setting("wicket.actor_id").await.expect("b");
    assert_eq!(b, ctx_b.actor.id.as_uuid().to_string());
    assert_ne!(a, b);
    tx.commit().await.expect("commit b");

    let pool = wicket_db::connect_with(
        &app_url(db.database()),
    ...
    .expect("probe");
    let leaked: (Option<String>,) =
        sqlx::query_as("SELECT pg_catalog.current_setting('wicket.actor_id', true)")
            .fetch_one(&pool)
            .await
...
/// D3 §10 row **d**: session-level `wicket.actor_id` set by hand on a raw
/// connection; the next write is refused by the `wicket.txid` check.
#[tokio::test]
async fn session_level_actor_is_refused() {
    let db = db_case!("session_actor");
    install_require_context_stub(db.migrate_pool()).await;
    let mut conn = connect_app_raw(db.database()).await;
    sqlx::query("SELECT pg_catalog.set_config('wicket.actor_id', $1, false)")
        .bind(wicket_core::Identifier::generate().as_uuid().to_string())
        .execute(&mut conn)
        .await
...
async fn after_release_resets_state() {
    ...
    let ctx = test_ctx();
    let mut tx = Tx::begin(&write, &ctx).await.expect("begin");
    tx.execute("SELECT pg_catalog.set_config('wicket.pool_probe', 'leaked', false)")
        .await
        .expect("session guc");
    tx.commit().await.expect("commit");

    let mut tx = Tx::begin(&write, &ctx).await.expect("begin 2");
    let leaked = tx.setting("wicket.pool_probe").await.expect("probe");
    assert!(
        leaked.is_empty(),
...
async fn refused_maps_42501() {
    ...
    tx.execute("SELECT pg_catalog.set_config('wicket.actor_id', '', true)")
        .await
        .expect("clear actor");
    let err = tx
