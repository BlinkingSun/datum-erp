...
pub fn hash_bytes(bytes: &[u8]) -> BlobHash {
    BlobHash(wicket_audit::sha256::digest(bytes))
}
...
impl FsBlobStore {
    ...
    /// Store under `WICKET_BLOB_ROOT`.
    pub fn from_env() -> Result<Self> {
        let root = std::env::var("WICKET_BLOB_ROOT").map_err(|_| Error::BlobRootMissing)?;
        if root.is_empty() {
...
impl BlobStore for FsBlobStore {
    ...
    fn verify(&self, hash: BlobHash) -> Result<()> {
        ...
        let got = wicket_audit::sha256::digest(&bytes);
        if got == hash.0 {
            Ok(())
        } else {
...
mod tests {
    ...
    fn tmp_store() -> FsBlobStore {
        let root = std::env::temp_dir().join(format!(
            "wicket-blob-unit-{}",
            wicket_core::Identifier::generate()
        ));
