...
//!
//! `wicket-print` calls these instead of selecting `documents.*` itself (R-2s-3).
...
use wicket_statemachine::{DocRef, current_state, current_state_on};
...
fn hash_manifest(content: &Value) -> Result<[u8; 32]> {
    ...
    Ok(wicket_audit::sha256::digest(&body))
}
