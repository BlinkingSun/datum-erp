...
use wicket_documents::{DOC_TYPE, FsBlobStore, document_machine};
use wicket_identity::rbac::{RoleBundle, assign_role, seed_bundles};
use wicket_identity::{PrincipalKind, create_principal, seed_builtins};
use wicket_statemachine::{DocRef, Engine};
...
pub async fn migrate(db: &wicket_test::TestDb) {
    ...
    wicket_module::order::install_upto(db.migrate_pool(), &boot, "wicket-documents")
        .await
        .unwrap_or_else(|e| panic!("install_upto wicket-documents: {e:#}"));
    boot.close().await;
...
pub fn read_pool_app(db: &wicket_test::TestDb) -> ReadPool {
    ReadPool::new(db.app_pool().clone())
}

pub fn read_pool_migrate(db: &wicket_test::TestDb) -> ReadPool {
...
pub async fn persist_engine(write: &WritePool, eng: &Engine, profile: &str) {
    let mut tx = wicket_db::Tx::begin(write, &write_ctx("documents.boot", profile))
        .await
...
pub async fn actor_with_docs(write: &WritePool, profile: &str) -> (Actor, WriteContext) {
    ...
    let short: String = slug
        ...
        .take(12)
        .collect();
    let mut tx = wicket_db::Tx::begin(write, &system_ctx("identity.create"))
        .await
        .expect("begin");
...
pub fn doc_ref(id: wicket_documents::DocumentId) -> DocRef {
    DocRef {
...
pub fn db_sqlstate(err: &wicket_db::Error) -> String {
    match err {
        wicket_db::Error::Refused(state) => state.as_str().to_owned(),
