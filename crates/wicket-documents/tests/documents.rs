...
use uuid::Uuid;
use wicket_core::{Identifier, NoSignatures};
use wicket_db::Tx;
...
async fn no_delete_privilege_on_documents() {
    for_each_profile(|profile| async move {
        ...
        for table in [
            ...
            let allowed: bool =
                query_scalar("SELECT has_table_privilege('wicket_app', $1, 'DELETE')")
                    .bind(table)
                    .fetch_one(db.app_pool())
                    .await
                    .unwrap();
            assert!(!allowed, "{table} must not grant DELETE to wicket_app");
        }
        let write = write_pool(&db);
        let eng = frozen_engine(profile);
        persist_engine(&write, &eng, profile).await;
    ...
    for_each_profile(|profile| async move {
        ...
        let a = new_revision(
            ...
            Manifest {
                ...
                from_precision: None,
                until_precision: Some(wicket_documents::DatePrecision::Day),
            },
        )
        .await
        .unwrap();
        let b = new_revision(
            ...
            Manifest {
                ...
                from_precision: Some(wicket_documents::DatePrecision::Day),
                until_precision: Some(wicket_documents::DatePrecision::Day),
            },
        )
        .await
...
async fn create_in_review(
    ...
    eng: &wicket_statemachine::Engine,
    profile: &str,
    gate: &dyn wicket_core::SignatureGate,
) -> wicket_documents::DocumentId {
    persist_engine(write, eng, profile).await;
    let (_, base) = actor_with_docs(write, profile).await;
    let mut tx = Tx::begin(write, &write_ctx("documents.create", profile))
        .await
...
async fn regulated_approve_refused_under_no_signatures_nothing_written() {
    for_each_profile(|profile| async move {
        ...
        assert!(
            matches!(
                err,
                Error::Signature(wicket_core::SignatureError::NoProvider)
            ),
            "got {err:?}"
        );
        tx.rollback().await.unwrap();
...
async fn every_write_audited_with_stamps() {
    for_each_profile(|profile| async move {
        ...
        let app_version = wicket_db::app_version();
...
async fn blob_not_orphaned_on_rollback() {
    for_each_profile(|profile| async move {
        ...
        let missing = attach(
            &mut tx,
            &store,
            wicket_documents::RevisionId(Identifier::generate()),
            bytes,
            "missing.bin",
            "application/octet-stream",
        )
