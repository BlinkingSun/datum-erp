...
`Engine::transition`, `wicket.esign_id` on audit rows, profile TOML flip) is the
...
| Table | Notes |
|---|---|
| `esign.signature` | Insert-only. `wicket_app` holds `SELECT`, `INSERT`, and `UPDATE (consumed_at, consumed_xid)` only (D-2b-1). A `BEFORE UPDATE` trigger refuses any other column; `consumed_at` is monotone. No `DELETE`. |
...
Tables are owned by `wicket_owner` (NOLOGIN). App-class tables are audited by
...
## Route shapes (transport is `wicket-server`)
...
gate = "NoSignatures"          # or "wicket-esign"
...
- `manifestation_for_record(tx, record)` / `manifestation_for_record_on(pool, record)` — D-2b-2 list by record version including supersession. **Consumer: `wicket-print`** (R-2s-3)
...
- `signature_consumed_at(tx, id)` / `signature_consumed_at_on(pool, id)` — `consumed_at`, or `None` if missing/unconsumed. **Consumer: `wicket-server`** (R-2s-3)
- `manifestation_for_record(tx, record)` / `manifestation_for_record_on(pool, record)` — D-2b-2 list by record version including live-version supersession. **Consumer: `wicket-print`** (R-2s-3)
...
Reads go through `wicket_db::ReadPool` (`fetch_one` / `fetch_optional` /
`fetch_all`) and, for the print and server seams, the sealed `Tx`. `archival_bundle` still
calls `wicket_audit::bundle`, which takes `&Pool`; that one call uses
`ReadPool::as_pool`.

## Published read seams

Kernel crates and `wicket-server` must not SELECT `esign.*` (CONTRACT §5a /
...
|---|---|---|---|
| `manifestation` | `async fn manifestation(pool: &ReadPool, id: SignatureId) -> Result<Manifestation>` | `esign.signature` + live `sm.instance.version` overlay (D-2b-7) | `wicket-server` GET `/esign/signatures/{id}` |
| `manifestation_in_tx` | `async fn manifestation_in_tx(tx: &mut Tx<'_>, id: SignatureId) -> Result<Manifestation>` | same, on the sealed `Tx` (mint before commit) | `wicket-server` POST `/esign/signatures` |
| `signature_consumed_at` | `async fn signature_consumed_at(tx: &mut Tx<'_>, id: SignatureId) -> Result<Option<DateTime<Utc>>>` | `esign.signature.consumed_at`; `None` if missing or unconsumed | `wicket-server` Required-edge pre-check |
...
| `manifestation_for_record` | `async fn manifestation_for_record(tx: &mut Tx<'_>, record: &RecordRef) -> Result<Vec<Manifestation>>` | same wire, filtered by `(record_table, record_id, record_version)`, oldest first | `wicket-print` |
| `manifestation_for_record_on` | `async fn manifestation_for_record_on(pool: &ReadPool, record: &RecordRef) -> Result<Vec<Manifestation>>` | same, through `ReadPool` (no actor) | `wicket-print` |
