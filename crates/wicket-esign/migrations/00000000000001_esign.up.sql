...
-- goes through invoker `esign.*` functions. Reversible. Owned by wicket_owner.
...
SELECT
  ...
  pg_catalog.set_config('wicket.action',        'esign.migrate', true),
  pg_catalog.set_config('wicket.source_kind',   'migration', true);

CREATE SCHEMA IF NOT EXISTS esign AUTHORIZATION wicket_migrate;

REVOKE ALL ON SCHEMA esign FROM PUBLIC;
GRANT USAGE ON SCHEMA esign TO wicket_app;
GRANT USAGE, CREATE ON SCHEMA esign TO wicket_migrate, wicket_owner;

INSERT INTO wicket.schema_class (nspname, class) VALUES ('esign', 'app')
ON CONFLICT (nspname) DO UPDATE SET class = EXCLUDED.class;

ALTER DEFAULT PRIVILEGES FOR ROLE wicket_migrate IN SCHEMA esign
  GRANT SELECT, INSERT, UPDATE ON TABLES TO wicket_app;
ALTER DEFAULT PRIVILEGES FOR ROLE wicket_migrate IN SCHEMA esign
  GRANT TRIGGER ON TABLES TO wicket_owner;
ALTER DEFAULT PRIVILEGES FOR ROLE wicket_owner IN SCHEMA esign
  GRANT SELECT, INSERT, UPDATE ON TABLES TO wicket_app;
...
ALTER TABLE esign.meaning_policy OWNER TO wicket_owner;
...
ALTER TABLE esign.signature OWNER TO wicket_owner;
...
ALTER TABLE transient.signing_session OWNER TO wicket_owner;
...
GRANT SELECT, INSERT ON TABLE esign.signature TO wicket_app;
REVOKE UPDATE ON TABLE esign.signature FROM wicket_app, PUBLIC;
GRANT UPDATE (consumed_at, consumed_xid) ON TABLE esign.signature TO wicket_app;
REVOKE DELETE ON TABLE esign.signature FROM wicket_app, PUBLIC;

GRANT SELECT, INSERT, UPDATE ON TABLE esign.meaning_policy TO wicket_app;
REVOKE DELETE ON TABLE esign.meaning_policy FROM wicket_app, PUBLIC;

GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE transient.signing_session TO wicket_app;

GRANT EXECUTE ON FUNCTION esign.refuse_immutable_update() TO wicket_app, wicket_migrate, wicket_owner;
...
LANGUAGE sql STABLE SET search_path = pg_catalog, pg_temp AS $fn$
  SELECT s.id, s.principal_id, s.login_session_id, s.device_fingerprint, s.boot_epoch,
   ...
   LIMIT 1
$fn$;
ALTER FUNCTION esign.load_open_signing_session(uuid) OWNER TO wicket_owner;
...
$fn$;
ALTER FUNCTION esign.open_signing_session(uuid, uuid, uuid, text, text, text) OWNER TO wicket_owner;
...
$fn$;
ALTER FUNCTION esign.touch_signing_session(uuid) OWNER TO wicket_owner;
...
END
$fn$;
ALTER FUNCTION esign.close_signing_sessions(uuid, text) OWNER TO wicket_owner;

GRANT EXECUTE ON FUNCTION esign.load_open_signing_session(uuid) TO wicket_app, wicket_migrate, wicket_owner;
GRANT EXECUTE ON FUNCTION esign.open_signing_session(uuid, uuid, uuid, text, text, text) TO wicket_app, wicket_migrate, wicket_owner;
GRANT EXECUTE ON FUNCTION esign.touch_signing_session(uuid) TO wicket_app, wicket_migrate, wicket_owner;
GRANT EXECUTE ON FUNCTION esign.close_signing_sessions(uuid, text) TO wicket_app, wicket_migrate, wicket_owner;
...
GRANT EXECUTE ON FUNCTION esign.supersede_signature(uuid, uuid) TO wicket_app, wicket_migrate, wicket_owner;
...
ALTER SCHEMA esign OWNER TO wicket_owner;
