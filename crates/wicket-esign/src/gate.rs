...
use uuid::Uuid;
use wicket_core::{
    Identifier, NoSignatures, RecordRef, SignatureError, SignatureGate, SignatureRequirement,
    SignatureToken,
};
use wicket_identity::{PrincipalStatus, UserId, load_principal_on};
...
impl GateFactory {
    ...
    /// `wicket-esign` binding (regulated-device from 2b.1).
    pub fn wicket_esign() -> Self {
        Self {
            kind: FactoryKind::WicketEsign,
        }
    }
