...
//! the transition's transaction through a prepared [`wicket_core::SignatureGate`].
//!
//! Composition-root wiring (`GateBinding` factory at startup, `prepare` before
//! `Engine::transition`, `wicket.esign_id` on audit rows, profile TOML flip) is
