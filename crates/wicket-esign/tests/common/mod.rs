...
use wicket_esign::{InstanceTriple, LiveDoc, MintRequest, SessionPolicy, Signature};
...
pub async fn migrate_esign(db: &wicket_test::TestDb) {
    let boot = db.bootstrap_pool().await.expect("bootstrap pool");
    wicket_module::order::install_upto(db.migrate_pool(), &boot, "wicket-esign")
        .await
        .unwrap_or_else(|e| panic!("install_upto wicket-esign: {e:#}"));
    boot.close().await;
...
pub async fn migrate_esign_sm(db: &wicket_test::TestDb) {
