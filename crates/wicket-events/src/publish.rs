...
pub async fn publish(tx: &mut wicket_db::Tx<'_>, event: Event) -> Result<Identifier> {
    crate::schema::global()?.validate(&event.name, event.version, &event.payload)?;

    let actor_raw = tx.setting("wicket.actor_id").await?;
    let actor_uuid = Uuid::parse_str(actor_raw.trim()).map_err(|e| {
        Error::Invariant(format!("wicket.actor_id is not a uuid ({actor_raw}): {e}"))
    })?;
    let source_kind = tx.setting("wicket.source_kind").await?;
    if source_kind.is_empty() {
        return Err(Error::Invariant(
            "wicket.source_kind is empty; begin through Tx::begin".into(),
        ));
    }
    ...
    let doc_type = match event.doc_type.clone() {
        ...
        _ => {
            let s = tx.setting("wicket.doc_type").await?;
    ...
    let doc_id = match event.doc_id {
        ...
        None => {
            let s = tx.setting("wicket.doc_id").await?;
            if s.is_empty() {
                None
            } else {
                Some(Uuid::parse_str(s.trim()).map_err(|e| {
                    Error::Invariant(format!("wicket.doc_id is not a uuid ({s}): {e}"))
                })?)
            }
        }
    };
