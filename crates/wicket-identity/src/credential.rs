...
pub async fn complete_reset(
    ...
    let user = UserId::from_identifier(wicket_core::Identifier::from_uuid(subject));
    match kind.as_str() {
...
async fn upsert_login_hash(tx: &mut wicket_db::Tx<'_>, principal: UserId, phc: &str) -> Result<()> {
    tx.execute(
        sql_query(
