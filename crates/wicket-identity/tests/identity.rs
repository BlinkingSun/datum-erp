...
use wicket_db::Tx;
use wicket_identity::rbac::{assign_role, has_permission, load_bundles, seed_bundles};
use wicket_identity::{
...
async fn username_is_never_reused() {
    ...
    let err = create_principal(&mut tx, PrincipalKind::User, "ada", "Ada 2")
        ...
        .expect_err("reuse");
    assert!(
        matches!(err, wicket_identity::Error::UsernameReused),
        "got {err:?}"
    );
...
async fn reset_requires_two_principals() {
    ...
    assert!(
        matches!(err, wicket_identity::Error::ResetRequiresTwoPrincipals),
        "got {err:?}"
    );
...
async fn lockout_after_n_failures() {
    ...
    for i in 0..LOCKOUT_AFTER {
        ...
        if i + 1 < LOCKOUT_AFTER {
            assert!(
                matches!(err, wicket_identity::Error::InvalidCredentials),
                "i={i} {err:?}"
            );
        } else {
            assert!(
                matches!(err, wicket_identity::Error::Lockout { .. }),
                "i={i} {err:?}"
            );
        }
