...
#[tokio::test]
async fn identity_tables_owned_by_wicket_owner() {
    ...
    for (table, owner, can_login) in owners {
        assert_eq!(owner, "wicket_owner", "{table}");
...
async fn builtins_exist_after_migrate() {
    ...
    .await
    .expect("rows");
    assert_eq!(rows[0].0, wicket_identity::MIGRATION_ID.to_string());
    assert_eq!(rows[0].1, "migration");
    assert_eq!(rows[0].2, "migration");
    assert_eq!(rows[1].0, wicket_identity::SYSTEM_ID.to_string());
