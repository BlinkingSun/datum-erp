...
ALTER TABLE transient.job OWNER TO wicket_owner;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE transient.job TO wicket_app;
...
ALTER TABLE app.run_log OWNER TO wicket_owner;
GRANT SELECT, INSERT, UPDATE ON TABLE app.run_log TO wicket_app;
...
ALTER TABLE transient.schedule OWNER TO wicket_owner;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE transient.schedule TO wicket_app;
...
-- Invoker: wicket_app holds DELETE on transient.job (D-W1-2). Kind name stays jobs.prune_finished.
...
END
$fn$;
ALTER FUNCTION transient.prune_finished(integer) OWNER TO wicket_owner;
REVOKE ALL ON FUNCTION transient.prune_finished(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION transient.prune_finished(integer) TO wicket_app, wicket_migrate, wicket_owner;
