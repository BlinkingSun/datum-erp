//! Bridge from [`wicket_events`] subscribers into the job queue.

use serde_json::json;
use wicket_db::Tx;
use wicket_events::{Event, EventHandler, HandlerFuture, Result as EventResult};

use crate::queue::{EnqueueOptions, enqueue};

const SUBSCRIBER: &str = "wicket-jobs";
...
/// Register the bridge with an events [`wicket_events::Registry`] and enable the subscription row.
pub async fn enable_genealogy_bridge(
    tx: &mut Tx<'_>,
    registry: &wicket_events::Registry,
) -> EventResult<()> {
    registry.subscribe(LOT_RECEIVED, SUBSCRIBER, GenealogyRefreshBridge);
    wicket_events::enable_subscription(tx, LOT_RECEIVED, SUBSCRIBER).await?;
    Ok(())
}
