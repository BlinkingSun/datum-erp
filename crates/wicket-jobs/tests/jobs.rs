...
use wicket_db::Tx;
use wicket_events::{Dispatcher, Event, SchemaRegistry, publish};
use wicket_jobs::{
...
impl JobHandler for FailHandler {
    fn run(
        ...
        Box<dyn std::future::Future<Output = wicket_jobs::Result<HandlerOutcome>> + Send + '_>,
    > {
        Box::pin(async move { Err(wicket_jobs::Error::Invariant("boom".into())) })
    }
}
...
async fn event_bridge_enqueues_job() {
    ...
    let events_reg = wicket_events::Registry::new();

    let mut tx = Tx::begin(&write, &ctx).await.expect("begin");
    wicket_jobs::events::enable_genealogy_bridge(&mut tx, &events_reg)
        .await
        .expect("bridge");
