...
-- `transient`. Reversible. Tables owned by wicket_owner. No ON DELETE CASCADE.
--
...
--   * stock_uom_id / posting.uom_id are bigint, not uuid: wicket-core UnitId is i64 and
...
SELECT
  ...
  pg_catalog.set_config('wicket.action',        'ledger.migrate', true),
  pg_catalog.set_config('wicket.source_kind',   'migration', true);

CREATE SCHEMA IF NOT EXISTS ledger AUTHORIZATION wicket_migrate;

REVOKE ALL ON SCHEMA ledger FROM PUBLIC;
GRANT USAGE ON SCHEMA ledger TO wicket_app;
GRANT USAGE, CREATE ON SCHEMA ledger TO wicket_migrate, wicket_owner;

INSERT INTO wicket.schema_class (nspname, class) VALUES ('ledger', 'app')
ON CONFLICT (nspname) DO UPDATE SET class = EXCLUDED.class;

ALTER DEFAULT PRIVILEGES FOR ROLE wicket_migrate IN SCHEMA ledger
  GRANT SELECT, INSERT, UPDATE ON TABLES TO wicket_app;
ALTER DEFAULT PRIVILEGES FOR ROLE wicket_migrate IN SCHEMA ledger
  GRANT TRIGGER ON TABLES TO wicket_owner;
ALTER DEFAULT PRIVILEGES FOR ROLE wicket_owner IN SCHEMA ledger
  GRANT SELECT, INSERT, UPDATE ON TABLES TO wicket_app;
ALTER DEFAULT PRIVILEGES FOR ROLE wicket_migrate IN SCHEMA ledger
...
GRANT USAGE ON TYPE ledger.group_kind TO wicket_app, wicket_migrate, wicket_owner;
GRANT USAGE ON TYPE ledger.measure TO wicket_app, wicket_migrate, wicket_owner;
GRANT USAGE ON TYPE ledger.boundary TO wicket_app, wicket_migrate, wicket_owner;
GRANT USAGE ON TYPE ledger.cost_element TO wicket_app, wicket_migrate, wicket_owner;
GRANT USAGE ON TYPE ledger.value_account TO wicket_app, wicket_migrate, wicket_owner;
...
ALTER TABLE ledger.stock_item OWNER TO wicket_owner;
...
ALTER TABLE ledger.location OWNER TO wicket_owner;
...
ALTER TABLE ledger.posting_group OWNER TO wicket_owner;
...
ALTER TABLE ledger.posting OWNER TO wicket_owner;
...
ALTER TABLE ledger.consumption OWNER TO wicket_owner;
...
END;
$fn$;
ALTER FUNCTION ledger.enforce_group_invariants() OWNER TO wicket_owner;
GRANT EXECUTE ON FUNCTION ledger.enforce_group_invariants() TO wicket_app, wicket_migrate, wicket_owner;
...
ALTER TABLE transient.balance_projection OWNER TO wicket_owner;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE transient.balance_projection TO wicket_app;
...
ALTER TABLE transient.layer_projection OWNER TO wicket_owner;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE transient.layer_projection TO wicket_app;

GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA ledger TO wicket_app;
REVOKE DELETE ON ALL TABLES IN SCHEMA ledger FROM PUBLIC, wicket_app;
GRANT USAGE ON ALL SEQUENCES IN SCHEMA ledger TO wicket_app;

GRANT SELECT (item_id) ON TABLE ledger.posting TO wicket_owner;
