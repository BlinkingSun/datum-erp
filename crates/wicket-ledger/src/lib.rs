...
//! deferred constraint trigger, [`GroupBuilder`] as [`wicket_core::PostingSink`],
...
use wicket_core::{DimensionKind, Identifier};
...
/// Seeded `uom.unit` ids (wicket-uom 0001) → dimension. Used when reconstructing
/// [`wicket_core::AnyQuantity`] from a posting row.
