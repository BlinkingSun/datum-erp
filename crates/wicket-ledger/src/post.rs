...
///
/// `wicket-db` does not expose `Tx::poison`. An unfinalized sink is detected here
/// (and in [`crate::commit`]) rather than by a GUC `wicket.ledger_unfinalized`.
...
async fn insert_finalized(tx: &mut Tx<'_>, builder: &GroupBuilder) -> Result<Identifier> {
    ...
    let actor_s = tx.setting("wicket.actor_id").await?;
    ...
    let mut explicit_by_handle: HashMap<u32, Vec<wicket_core::ConsumptionPosting>> = HashMap::new();
...
fn sum_values_for(
    ...
    for (h, v) in values {
        if v.values == Some(handle) {
            acc = Some(match acc {
                Some(a) => a.try_add(v.amount).map_err(wicket_core::Error::from)?,
                None => v.amount,
            });
        }
        let _ = h;
    }
    Ok(acc.unwrap_or_else(|| Money::zero(wicket_core::CurrencyId(840))))
}
