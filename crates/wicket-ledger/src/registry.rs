...
use wicket_core::{Boundary, CurrencyId, Identifier, ItemId, LocationId, Money, UnitId};
use wicket_db::Tx;
...
pub async fn load_stock_item(tx: &mut Tx<'_>, item: ItemId) -> Result<StockItem> {
    ...
    let standard = match (sc, scur) {
        (Some(amount), Some(cur)) => {
            Some(Money::new(amount, CurrencyId(i32::from(cur))).map_err(wicket_core::Error::from)?)
        }
        _ => None,
    };
    Ok(StockItem {
