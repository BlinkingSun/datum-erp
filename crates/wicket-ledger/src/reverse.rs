...
pub async fn reverse(tx: &mut Tx<'_>, group_id: Identifier, reason: &str) -> Result<Identifier> {
    ...
    for (pid, _m, item, uom, loc, boundary, lot, serial, qty, entered, entered_uom) in qtys {
        ...
        let handle = builder
            .contribute(PostingIntent::Quantity(QuantityPosting {
                item: wicket_core::ItemId::from_uuid(item),
                quantity: AnyQuantity {
                    amount: -qty,
                    unit: wicket_core::UnitId(uom),
                    dimension: dim,
                },
                location: wicket_core::LocationId::from_uuid(loc),
                boundary: boundary.as_deref().and_then(|s| boundary_from_sql(s).ok()),
                lot: lot.map(wicket_core::LotId::from_uuid),
                serial: serial.map(wicket_core::SerialId::from_uuid),
                entered: entered.map(|e| AnyQuantity {
                    amount: -e,
                    unit: wicket_core::UnitId(entered_uom.unwrap_or(uom)),
                    dimension: dim,
                }),
            }))
    ...
    for (account, element, cost_object, currency, amount, values_pid) in vals {
        ...
        builder
            .contribute(PostingIntent::Value(ValuePosting {
                ...
                amount: Money::new(-amount, wicket_core::CurrencyId(i32::from(currency)))
    ...
    for (consuming, consumed, qty, amt) in cons {
        ...
        let dim = wicket_core::DimensionKind::Count;
        builder
            .contribute(PostingIntent::Consumption(
                wicket_core::ConsumptionPosting {
                    ...
                    amount: Money::new(-amt, wicket_core::CurrencyId(840))
                        .map_err(wicket_core::Error::from)?,
                },
            ))
            .map_err(Error::from_posting)?;
