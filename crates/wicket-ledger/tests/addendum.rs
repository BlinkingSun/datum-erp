...
async fn explicit_consumption_skips_auto_allocation() {
    let db = wicket_test::db_case!("p3ex");
...
async fn unfinalized_box_dyn_posting_sink_poisons() {
    let db = wicket_test::db_case!("unfin_box");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn unfinalized_mut_dyn_posting_sink_poisons() {
    let db = wicket_test::db_case!("unfin_mut");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn group_builder_drop_does_not_poison_without_contributed_mark() {
    let db = wicket_test::db_case!("drop_no_contrib");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn poisoned_transaction_cannot_commit() {
    let db = wicket_test::db_case!("poison_commit");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn standard_costing_posts_ppv() {
    let db = wicket_test::db_case!("std_ppv");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn loc_simple(tx: &mut Tx<'_>, boundary: Option<Boundary>) -> wicket_core::LocationId {
    let id = wicket_core::LocationId::generate();
    wicket_ledger::upsert_location(tx, id, boundary)
        .await
        .unwrap();
    id
}
...
async fn balance_at_is_lot_and_serial_aware() {
    let db = wicket_test::db_case!("bal_lot");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn reverse_kind_from_target_not_sink() {
    let db = wicket_test::db_case!("revkind");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn no_reversal_of_reversal() {
    let db = wicket_test::db_case!("norev");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn ineligible_layer_rejected() {
    let db = wicket_test::db_case!("inelig");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn explicit_consumption_money_half_validated() {
    let db = wicket_test::db_case!("ex_money");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn verify_projection_catches_each_column_poison() {
    let db = wicket_test::db_case!("vproj");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn allocation_is_per_location() {
    let db = wicket_test::db_case!("p3loc");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn apply_group_incremental_matches_rebuild() {
    let db = wicket_test::db_case!("incrb");
    common::migrate(&db).await;
    let pool = write_pool(&db);
