...
use wicket_ledger::{CostMethod, GroupBuilder, upsert_location, upsert_stock_item};
...
/// D-2b-10 order through `wicket-ledger`. Glue rider flips this to
/// `wicket_module::order::install_upto(..., "wicket-ledger")`.
...
/// Predecessors of `wicket-ledger` in D-2b-10 order, event trigger already up.
pub async fn migrate_predecessors(db: &wicket_test::TestDb) {
    ...
    .await
    .expect("canonical predecessors of wicket-ledger");
}
...
pub async fn migrate_as_last_crate(db: &wicket_test::TestDb) {
    ...
    .await
    .expect("wicket-ledger last with audit_attach up");
}
...
pub fn pg_code_db(err: &wicket_db::Error) -> String {
    match err {
        ...
        wicket_db::Error::Sqlx(e) => pg_code_sqlx(e),
        other => other.to_string(),
    }
}

pub fn corruption_code_ledger(err: &wicket_ledger::Error) -> String {
    use wicket_core::PostingError;
    match err {
        wicket_ledger::Error::Posting(PostingError::IneligibleLayer { .. }) => {
            "IneligibleLayer".into()
        }
        wicket_ledger::Error::Posting(PostingError::AllocationRequired(_)) => {
...
pub fn pg_code_ledger(err: &wicket_ledger::Error) -> String {
    match err {
        wicket_ledger::Error::GroupHasNoHeader => "ZL000".into(),
        wicket_ledger::Error::GroupExtended => "ZL001".into(),
        wicket_ledger::Error::QuantityNotConserved => "ZL002".into(),
        wicket_ledger::Error::ValueNotConserved => "ZL003".into(),
        wicket_ledger::Error::CostElementReclassified => "ZL004".into(),
        wicket_ledger::Error::AllocationIncomplete => "ZL005".into(),
        wicket_ledger::Error::ReversalNotExact => "ZL006".into(),
        wicket_ledger::Error::LayersNotRestored => "ZL007".into(),
        wicket_ledger::Error::AlreadyReversed => "23505".into(),
        wicket_ledger::Error::ParentMustExist => "23503".into(),
        wicket_ledger::Error::Db(e) => pg_code_db(e),
        other => other.to_string(),
    }
}
...
pub async fn post_case_a(tx: &mut Tx<'_>, w: &World) -> Identifier {
    use wicket_core::{GroupKind, PostingIntent, PostingSink};
    ...
    .unwrap();
    wicket_ledger::post(tx, b).await.expect("case a")
}
...
pub async fn post_case_b(tx: &mut Tx<'_>, w: &World) -> Identifier {
    ...
    .unwrap();
    wicket_ledger::post(tx, b).await.expect("case b")
}
...
pub async fn seed_screws_fg(tx: &mut Tx<'_>, w: &World) {
    ...
    .unwrap();
    wicket_ledger::post(tx, b).await.expect("seed screws");
}
...
pub async fn post_case_c(tx: &mut Tx<'_>, w: &World) -> Identifier {
    ...
    .unwrap();
    wicket_ledger::post(tx, b).await.expect("case c")
}

pub async fn actor_uuid(tx: &mut Tx<'_>) -> uuid::Uuid {
    uuid::Uuid::parse_str(&tx.setting("wicket.actor_id").await.expect("actor")).expect("uuid")
}
...
pub async fn raw_group(
    ...
    let gid = wicket_core::Identifier::generate().as_uuid();
...
pub async fn post_gen_internal_move(tx: &mut Tx<'_>, w: &World, feet: &str) -> Identifier {
    ...
    .unwrap();
    wicket_ledger::post(tx, b).await.expect("gen move")
}
...
pub async fn post_gen_adjustment(tx: &mut Tx<'_>, w: &World) -> Identifier {
    ...
    .unwrap();
    wicket_ledger::post(tx, b).await.expect("gen adj")
}
...
pub async fn post_gen_transformation(tx: &mut Tx<'_>, w: &World) -> Identifier {
    use wicket_core::{CostElement, GroupKind, PostingIntent, PostingSink, ValuePosting};
    ...
    .unwrap();
    wicket_ledger::post(tx, b).await.expect("gen xform")
}
...
pub async fn post_gen_screw_release(tx: &mut Tx<'_>, w: &World) -> Identifier {
    ...
    .unwrap();
    wicket_ledger::post(tx, b).await.expect("gen screw issue")
}
