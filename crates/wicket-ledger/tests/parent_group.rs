...
use uuid::Uuid;
use wicket_core::{Boundary, GroupKind, Identifier, PostingIntent, PostingSink};
use wicket_db::Tx;
use wicket_ledger::{Error, GroupBuilder, UOM_CONVERSION_RESIDUAL, children_of, post};

use common::*;

#[tokio::test]
async fn residual_group_links_to_parent() {
    let db = wicket_test::db_case!("pg_residual");
...
async fn parent_must_exist() {
    let db = wicket_test::db_case!("pg_fk");
    common::migrate(&db).await;
    let pool = write_pool(&db);
...
async fn children_of_no_actor_is_refused() {
    let db = wicket_test::db_case!("pg_42501");
    common::migrate(&db).await;
