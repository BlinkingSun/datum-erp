...
pub struct ModuleManifest {
    ...
    #[serde(default, skip)]
    pub custom_fields: wicket_customfields::ManifestCustomFields,
...
impl ModuleManifest {
    ...
    pub fn manifest_hash(&self, enabled: bool) -> Result<String> {
        Ok(hex(wicket_audit::sha256::digest(
...
fn parse_custom_fields(
    ...
    owner: &str,
) -> Result<wicket_customfields::ManifestCustomFields> {
    let mut fields = Vec::new();
    for table in table_array(root, "custom-fields")? {
        let type_name = toml::require_str(table, "type")?;
        let field_type = wicket_customfields::FieldType::parse(&type_name)
        ...
        let field_owner = table
            ...
            .unwrap_or(owner)
            .to_string();
        fields.push(wicket_customfields::ManifestCustomField {
    ...
    Ok(wicket_customfields::ManifestCustomFields { fields })
}
