...
/// Which [`wicket_core::SignatureGate`] the composition root binds (key 4).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum GateBinding {
    /// `NoSignatures` (pre-Wave-2b and tests).
    NoSignatures,
    /// `wicket-esign` (Wave 2b).
    WicketEsign,
}
...
impl GateBinding {
    fn parse(s: &str) -> Result<Self> {
        match s {
            ...
            "wicket-esign" => Ok(Self::WicketEsign),
...
impl NumberingSpec {
    /// Map to [`wicket_numbering::ResetPolicy`].
    pub fn reset_policy(&self) -> Result<wicket_numbering::ResetPolicy> {
        match self.reset.as_str() {
            "never" => Ok(wicket_numbering::ResetPolicy::Never),
            "yearly" => Ok(wicket_numbering::ResetPolicy::Yearly),
            "monthly" => Ok(wicket_numbering::ResetPolicy::Monthly),
...
pub struct Profile {
    ...
    pub session_policy: wicket_esign::SessionPolicy,
    /// Key 5.
...
fn parse_session_policy(gate_tbl: &BTreeMap<String, Value>) -> Result<wicket_esign::SessionPolicy> {
    let defaults = wicket_esign::SessionPolicy::default();
