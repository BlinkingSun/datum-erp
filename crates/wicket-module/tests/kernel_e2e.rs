//! Finding 2: composed kernel path as `wicket-module` would drive it.
...
use wicket_db::Tx;
use wicket_esign::{InstanceTriple, LiveDoc, MintRequest, identity_projection, mint};
use wicket_identity::{PrincipalStatus, deactivate_principal};
use wicket_ledger::{CostMethod, rebuild, upsert_location, upsert_stock_item, verify_projection};
use wicket_statemachine::{DocRef, EdgeBuilder, Machine, Veto};
use wicket_test::db_case;

use wicket_module::{Error, KERNEL_ORDER, Kernel, Profile, SignatureEdge};
...
fn veto_of(err: wicket_core::PostingError) -> Veto {
    Veto {
...
fn dummy_token(doc: &DocRef, version: i64, actor: wicket_core::Actor) -> SignatureToken {
    SignatureToken {
...
async fn assert_migrators_and_history(db: &wicket_test::TestDb) {
    assert!(
        has_zz_audit(db.migrate_pool(), "wicket", "schema_history").await,
        "schema_history attached after install_upto"
    );
    let crates: Vec<String> =
        sqlx::query_scalar("SELECT DISTINCT crate FROM wicket.schema_history")
...
async fn assert_builtins(db: &wicket_test::TestDb) {
...
async fn composed_path(db: &wicket_test::TestDb, profile: Profile, rebuild_projections: bool) {
    ...
    assert!(
        matches!(frozen, wicket_statemachine::Error::Frozen),
        "got {frozen:?}"
    );
    ...
    let err = wicket_uom::update_item_stock(
        &mut tx,
        item,
    ...
    assert!(
        matches!(err, wicket_uom::Error::StockMeasureImmutable),
        "got {err:?}"
    );
    ...
    let ran = kernel
        ...
        .await
        .expect("worker");
    assert_eq!(ran, 1, "one job through wicket-jobs");
    let done: i64 =
...
async fn kernel_e2e_regulated_release_with_real_signature() {
    ...
    assert!(
        !kernel.gate_is_noop(),
        "regulated-device binds wicket-esign"
    );
...
async fn regulated_release_refused_without_signature_succeeds_with_two_component_signature() {
    ...
    assert!(
        matches!(
            dummy_err,
            wicket_statemachine::Error::Signature(SignatureError::Invalid(_))
        ),
...
async fn kernel_e2e_regulated_document_approve_with_real_signature() {
    ...
    let mut manifest = wicket_documents::Manifest::content(json!({}));
    manifest.effective_from = Some(from);
    manifest.from_precision = Some(wicket_documents::DatePrecision::Day);
    ...
    assert!(
        matches!(
            ...
            )) | wicket_module::Error::Documents(wicket_documents::Error::Signature(
                SignatureError::NoProvider | SignatureError::Invalid(_)
            ))
        ),
    ...
    let after_refuse = wicket_documents::load(&mut tx, id)
        .await
        .expect("load after refuse");
    tx.rollback().await.ok();
    assert_eq!(
        after_refuse.status,
        wicket_documents::Status::InReview,
    ...
    let live = wicket_documents::effective_at(&mut tx, id, chrono::Utc::now())
        .await
        .expect("effective_at");
...
async fn mint_document_token(
    kernel: &Kernel,
    write: &wicket_db::WritePool,
    principal: &wicket_identity::Principal,
    doc: &DocRef,
    meaning: &str,
    permission: &str,
) -> SignatureToken {
...
async fn mint_for_doc(
    ...
    principal: &wicket_identity::Principal,
    doc: &DocRef,
) -> (SignatureToken, wicket_core::RecordRef, i64) {
    let mut mint_tx = Tx::begin(write, &boot_ctx()).await.expect("mint begin");
    let loaded = kernel
...
/// field may not. wicket-module e2e (not server HTTP): Kernel::live_doc is the
...
async fn live_doc_hash_covers_registered_projection() {
    ...
    assert!(
        matches!(
            err,
            Error::Statemachine(wicket_statemachine::Error::Signature(
...
/// wicket-module e2e (not server HTTP): the live hash is Kernel::live_doc, and
/// this lane may only touch the mint hunk in wicket-server.
#[tokio::test]
async fn mint_then_body_edit_is_hash_mismatch() {
    ...
    assert_eq!(
        err.to_string(),
        "signature content hash mismatch",
        "kernel error that maps to HTTP 409 CONFLICT (D-2b-8 / wicket-server from_signature)"
    );
