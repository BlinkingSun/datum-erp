...
async fn migrate_down_then_up() {
    ...
    install_upto(db.migrate_pool(), &boot, "wicket-print")
        .await
    ...
    let class_gone: bool = sqlx::query_scalar(
        "SELECT NOT EXISTS (SELECT 1 FROM wicket.schema_class WHERE nspname = 'module')",
    )
    .fetch_one(db.migrate_pool())
    .await
    .expect("class gone");
...
async fn wave_2s1_migrate_down_then_up() {
    ...
    let boot = db.bootstrap_pool().await.expect("bootstrap");
    install_upto(db.migrate_pool(), &boot, "wicket-module")
        .await
        .expect("install_upto through wicket-module");
    boot.close().await;

    let order = wicket_module::wave_2s1_order().expect("order");
    assert_eq!(
        order,
        vec![
            "mod-items".to_string(),
    ...
    for (name, migrator) in wicket_module::wave_2s1_migrators().expect("migrators") {
        let rel = match name {
            "wicket-mod-items" => "items.item",
            "wicket-mod-locations" => "locations.site",
            "wicket-mod-lots" => "lots.lot",
...
/// The bootstrap pair has no event trigger until `wicket-audit` itself is applied,
...
async fn install_crate_last(migrate: &sqlx::PgPool, bootstrap: &sqlx::PgPool, crate_name: &str) {
    if matches!(crate_name, "wicket-db" | "wicket-audit") {
    ...
    wicket_audit::install_privileged(bootstrap)
        .await
