...
use wicket_events::EventHandler;
use wicket_statemachine::{DocRef, EdgeBuilder, Engine, HookPhase, Machine, ModuleNode};
...
fn kernel_order_is_a_topological_sort_of_contract_graph() {
    ...
    assert_eq!(KERNEL_ORDER[0], "wicket-db");
    assert_eq!(KERNEL_ORDER[1], "wicket-audit");
    assert!(KERNEL_ORDER.contains(&"wicket-esign"));
    assert!(KERNEL_ORDER.contains(&"wicket-customfields"));
    assert!(KERNEL_ORDER.contains(&"wicket-documents"));
    assert!(KERNEL_ORDER.contains(&"wicket-print"));
    let pos = |name: &str| KERNEL_ORDER.iter().position(|n| *n == name).unwrap();
    assert!(pos("wicket-esign") < pos("wicket-documents"));
    assert!(pos("wicket-customfields") < pos("wicket-documents"));
    assert!(pos("wicket-numbering") < pos("wicket-documents"));
    assert!(pos("wicket-statemachine") < pos("wicket-documents"));
    assert!(pos("wicket-documents") < pos("wicket-print"));
    assert_eq!(*KERNEL_ORDER.last().unwrap(), "wicket-module");
}
...
fn canonical_order_is_a_topological_sort_of_contract_graph() {
    ...
    assert_eq!(CANONICAL_ORDER[0], "wicket-db");
    assert_eq!(*CANONICAL_ORDER.last().unwrap(), "wicket-server");
    assert!(pos("wicket-module") < pos("wicket-mod-items"));
    assert!(pos("wicket-mod-items") < pos("wicket-mod-lots"));
    assert!(pos("wicket-mod-locations") < pos("wicket-mod-lots"));
    assert!(pos("wicket-mod-lots") < pos("wicket-mod-inventory"));
    assert!(pos("wicket-mod-inventory") < pos("wicket-mod-production-min"));
    assert!(pos("wicket-mod-genealogy") < pos("wicket-server"));
}
...
audit_export_cli = "wicket audit export"
...
fn regulated_device_startup_fails_if_required_meets_no_signatures() {
    ...
    assert_eq!(
        ...
        GateBinding::WicketEsign,
        "regulated-device binds wicket-esign so a live release boot is not this failure"
    );
...
fn hook_order_matches_statemachine_hook_order() {
    ...
    let ours = topological_order(
        &nodes
            .iter()
            .map(|n| wicket_module::ModuleNode {
                id: n.id.clone(),
...
async fn schema_history_trigger_is_present() {
    ...
    assert!(
        has_zz_audit(db.migrate_pool(), "wicket", "schema_history").await,
        "wicket.schema_history must be audited after attach"
    );
    ...
    for (label, profile) in [
        ...
        let discovered: Vec<String> = sqlx::query_scalar(
            ...
            SELECT n.nspname::text || '.' || c.relname::text
              ...
              JOIN wicket.schema_class sc ON sc.nspname = n.nspname
             WHERE sc.class = 'app'
...
async fn documents_schema_installed_with_kernel() {
    ...
    assert_eq!(n, 5, "install_kernel must apply wicket-documents");
    let class: String =
        sqlx::query_scalar("SELECT class FROM wicket.schema_class WHERE nspname = 'documents'")
            .fetch_one(db.migrate_pool())
            .await
            .expect("schema_class");
    assert_eq!(class, "app");
...
async fn documents_events_emitted_from_composition_root() {
    ...
    assert!(
        kernel
            .event_schemas
            .get(wicket_documents::EVENT_REVISION_CREATED, 1)
            .is_some(),
    ...
    assert!(
        kernel
            .event_schemas
            .get(wicket_documents::EVENT_EFFECTIVE, 1)
            .is_some(),
    ...
    let rev = kernel
        .new_document_revision(
            ...
            id,
            "A",
            wicket_documents::Manifest::content(serde_json::json!({})),
        )
        .await
        .expect("revision");
    ...
    let created: i64 =
        sqlx::query_scalar("SELECT count(*) FROM app.event WHERE name = $1 AND doc_id = $2")
            .bind(wicket_documents::EVENT_REVISION_CREATED)
            .bind(id.as_uuid())
            .fetch_one(db.app_pool())
            .await
    ...
    let effective: i64 =
        sqlx::query_scalar("SELECT count(*) FROM app.event WHERE name = $1 AND doc_id = $2")
            .bind(wicket_documents::EVENT_EFFECTIVE)
            .bind(id.as_uuid())
            .fetch_one(db.app_pool())
            .await
...
impl EventHandler for DummySubscriber {
    fn handle<'a, 'p: 'a>(
        &'a self,
        _tx: &'a mut Tx<'p>,
        _event: &'a wicket_events::Event,
    ) -> wicket_events::HandlerFuture<'a> {
        Box::pin(async { Ok(()) })
    }
}
...
fn signature_gate_comes_from_profile_toml_gate_field() {
    ...
    let plain = Profile::plain_shop().unwrap();
    assert_eq!(regulated.signature_gate_binding, GateBinding::WicketEsign);
...
async fn regulated_required_set_from_registered_machine() {
    ...
    assert!(
        kernel.profile.signature_edges.iter().any(
            |e| matches!(e, wicket_module::SignatureEdge::Required { edge, .. } if edge == "approve")
        ),
        "calibration.certificate.approve must be listed"
    );
    assert!(
        !kernel.gate_is_noop(),
        "regulated-device binds the wicket-esign factory"
    );
...
async fn module_manifest_custom_fields_registered() {
    ...
    let write = kernel.write_pool();
    let mut tx = Tx::begin(&write, &boot_ctx()).await.expect("begin");
    let defs = wicket_customfields::definitions_for(&mut tx, "items.item")
        .await
...
async fn enable_module_the_profile_disallows_is_refused() {
    ...
    assert!(
        matches!(err, wicket_module::Error::EnableRefused { .. }),
        "typed error, got {err}"
    );
    tx.rollback().await.ok();
    assert_eq!(
...
const SEEDED_PRINT_TEMPLATES: [&str; 3] = [
    wicket_print::TemplateId::DOCUMENT_REVISION,
    wicket_print::TemplateId::GENERIC_RECORD,
    wicket_print::TemplateId::WORK_ORDER_TRAVELER,
];

async fn print_install_profile(db: &wicket_test::TestDb) -> String {
...
async fn print_template_rows(db: &wicket_test::TestDb) -> Vec<(String, i32, Vec<u8>)> {
    sqlx::query_as(
...
async fn kernel_build_seeds_print_templates() {
    for (label, profile) in [
        ...
        let retire = kernel
            ...
            .edges_for_manifest()
            .into_iter()
            .find(|e| e.doc_type == wicket_customfields::DOC_TYPE && e.edge == "retire");
        assert!(
            retire.is_some(),
...
async fn kernel_assemble_registers_definition_machine() {
    for (label, profile) in [
        ...
        let edge = kernel
            ...
            .into_iter()
            .find(|e| {
                e.doc_type == wicket_customfields::DOC_TYPE
                    && e.edge == wicket_customfields::RETIRE_EDGE
            })
        ...
        assert!(
            kernel.profile.signature_edges.iter().any(|e| matches!(
                e,
                wicket_module::SignatureEdge::NotRequired { module, edge, .. }
                    if module == wicket_customfields::DOC_TYPE
                        && edge == wicket_customfields::RETIRE_EDGE
            )),
        ...
        let persisted: i64 =
            sqlx::query_scalar("SELECT count(*) FROM sm.machine WHERE doc_type = $1")
                .bind(wicket_customfields::DOC_TYPE)
                .fetch_one(db.app_pool())
                .await
                .expect("sm.machine");
        ...
        let write = kernel.write_pool();
        let actor = actor_with_perms(&write, &[wicket_customfields::RETIRE_PERMISSION]).await;
        ...
        let id = wicket_customfields::define(
            &mut tx,
            wicket_customfields::DefinitionSpec {
                entity: "items.item".into(),
                key: format!("w3b_{}", &label[..5]),
                field_type: wicket_customfields::FieldType::String,
                label: "Wave 3b".into(),
        ...
        let mut retire_ctx = kernel.transition_context(
            actor,
            &wicket_customfields::doc_ref(id),
            wicket_customfields::RETIRE_EDGE,
        );
        ...
        wicket_customfields::retire(&mut tx, &kernel.engine, id, &retire_ctx)
            .await
