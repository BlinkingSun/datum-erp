...
`wicket_documents` / `wicket_esign` render reads (`revision_for_render`,
...
| `archive` | Store bytes via caller-supplied `wicket_documents::BlobStore` (same pattern as `documents::attach`); idempotent per output hash. No process-global blob root. |
| `manifestation_block` | `wicket_esign::manifestation_for_record` (D-2b-2 snapshots, no live identity join) |
| `log` | List `render_log` rows for a record version |
| `set_installation_profile` | Boot stamp for 11.50(b) gating |

## Published read seams

Kernel crates and `wicket-server` must not SELECT `print.*` (CONTRACT §5a /
...
| `list_templates_on` | `async fn list_templates_on(pool: &ReadPool, profile: &str) -> Result<Vec<TemplateSummary>>` | same, through `ReadPool` (no actor) | `wicket-server` GET `/api/v1/print/templates` |
