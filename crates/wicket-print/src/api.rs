//! Public API on the sealed [`wicket_db::Tx`].
...
pub async fn render(
    ...
    let app_version = tx.setting("wicket.app_version").await?;
    let config_version = tx.setting("wicket.config_version").await?;
...
///
/// The caller supplies the [`BlobStore`] (same pattern as [`wicket_documents::attach`]).
/// Tests pass a unique [`wicket_documents::FsBlobStore`] rooted in a temp directory;
...
///
/// `wicket.config_version` is the profile *spec* version and must not be used as the
