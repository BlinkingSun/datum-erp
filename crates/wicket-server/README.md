...
HTTP API process for Wicket. Owns `crates/wicket-server/**` from Wave 2s (PLAN §3).
...
```
wicket serve --profile <regulated-device|plain-shop> --bind 0.0.0.0:8080
wicket migrate
wicket db check
wicket iq
...
Configuration is a TOML file (`--config` / `WICKET_CONFIG`) plus environment.
Database URLs are D3's names: `WICKET_DATABASE_URL`, `WICKET_MIGRATE_DATABASE_URL`,
`WICKET_BOOTSTRAP_URL`. The content-addressed blob store root is `WICKET_BLOB_ROOT`
(the same name `wicket_documents::FsBlobStore::from_env` already uses). It is
...
| File | Platform |
|---|---|
| `wicket.service` | systemd |
| `com.wicket.server.plist` | launchd |
| `install-windows-service.ps1` | `sc create` around the binary |

**Wicket never owns the database.** PostgreSQL is an external cluster. These unit
...
| Dummy / invalid token while `wicket-esign` is bound | `SIGNATURE_REQUIRED` | 403 |
...
Regulated-device binds `wicket-esign`, so a dummy token is **403 `SIGNATURE_REQUIRED`**, not 409 — 409 is only `NoProvider`. Plain-shop stays `NoSignatures` and enables no `Required` edge. Clients send the minted id as `X-Wicket-Signature` (docs/10 §5.2). `POST /esign/signatures` records the idempotency replay in the **same** mint transaction.
...
`wicket_mod_inventory::reverse_posted_issue`. **201** is the issue document plus
