...
`core::PostingSink` and `core::SignatureGate`; never depends on `wicket-ledger` or
`wicket-esign` (CONTRACT §4).
...
- `action_for` / `with_action` — `wicket.action` strings for the audit trigger
...
exempt this crate): `wicket_app` has `SELECT` on `sm.instance` / `sm.machine`.
`EXECUTE` is granted only to `wicket_app` (`REVOKE` from `PUBLIC`). They do **not**
call `audit.require_context`, so a `wicket_db::ReadPool` fetch (no actor) succeeds.

### Write seam (R-2s-5)

`wicket_app` has **no** `INSERT` / `UPDATE` / `DELETE` on `sm.instance`. A raw
...
`wicket_app` only). Those helpers DML `sm.instance_engine`, a `wicket_owner` view of
...
| Relation / function | `wicket_app` SELECT | INSERT | UPDATE | DELETE | EXECUTE |
|---|---|---|---|---|---|
...
- `00000000000002_query_seam` — `sm.current_state(text, uuid)`,
  ...
  `GRANT EXECUTE` to `wicket_app` only)
- `00000000000003_engine_write` — revoke `INSERT`/`UPDATE`/`DELETE` on
  `sm.instance` from `wicket_app`; `sm.instance_engine` view; `sm.spawn_instance`,
...
(`&mut dyn PostingSink`); `cargo tree` must not show `wicket-ledger`. Real
`SignatureGate` is Wave 2b `wicket-esign`. `Kernel::build` currently freezes with
