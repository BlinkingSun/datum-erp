...
-- Tables owned by wicket_owner. Reversible.
...
SELECT
  ...
  pg_catalog.set_config('wicket.action',        'statemachine.migrate', true),
  pg_catalog.set_config('wicket.source_kind',   'migration', true);

CREATE SCHEMA IF NOT EXISTS sm AUTHORIZATION wicket_migrate;

REVOKE ALL ON SCHEMA sm FROM PUBLIC;
GRANT USAGE ON SCHEMA sm TO wicket_app;
GRANT USAGE, CREATE ON SCHEMA sm TO wicket_migrate, wicket_owner;

ALTER DEFAULT PRIVILEGES FOR ROLE wicket_migrate IN SCHEMA sm
  GRANT SELECT, INSERT, UPDATE ON TABLES TO wicket_app;
ALTER DEFAULT PRIVILEGES FOR ROLE wicket_migrate IN SCHEMA sm
  GRANT TRIGGER ON TABLES TO wicket_owner;
ALTER DEFAULT PRIVILEGES FOR ROLE wicket_owner IN SCHEMA sm
  GRANT SELECT, INSERT, UPDATE ON TABLES TO wicket_app;
...
ALTER TABLE sm.machine OWNER TO wicket_owner;
GRANT SELECT, INSERT, UPDATE ON TABLE sm.machine TO wicket_app;
...
ALTER TABLE sm.state OWNER TO wicket_owner;
GRANT SELECT, INSERT, UPDATE ON TABLE sm.state TO wicket_app;
...
ALTER TABLE sm.edge OWNER TO wicket_owner;
GRANT SELECT, INSERT, UPDATE ON TABLE sm.edge TO wicket_app;
...
ALTER TABLE sm.instance OWNER TO wicket_owner;
GRANT SELECT, INSERT, UPDATE ON TABLE sm.instance TO wicket_app;

ALTER SCHEMA sm OWNER TO wicket_owner;
