-- 0003_engine_write: sm.instance DML for wicket_app is SELECT only. Spawn and
-- transition go through invoker-rights helpers (NOT SECURITY DEFINER — R-2s-8
-- does not exempt this crate). The helpers write a wicket_owner view so the
...
REVOKE INSERT, UPDATE, DELETE ON TABLE sm.instance FROM PUBLIC, wicket_app;
GRANT SELECT ON TABLE sm.instance TO wicket_app;

CREATE VIEW sm.instance_engine AS
  SELECT doc_type, doc_id, machine_id, state, version, entered_at
    FROM sm.instance;
ALTER VIEW sm.instance_engine OWNER TO wicket_owner;
REVOKE ALL ON sm.instance_engine FROM PUBLIC;
GRANT SELECT, INSERT, UPDATE ON sm.instance_engine TO wicket_app;
...
END
$fn$;
ALTER FUNCTION sm.instance_engine_guard() OWNER TO wicket_owner;
...
END
$fn$;
ALTER FUNCTION sm.spawn_instance(text, uuid, uuid, text) OWNER TO wicket_owner;
REVOKE ALL ON FUNCTION sm.spawn_instance(text, uuid, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION sm.spawn_instance(text, uuid, uuid, text) TO wicket_app;
...
END
$fn$;
ALTER FUNCTION sm.transition_instance(text, uuid, text, text, bigint) OWNER TO wicket_owner;
REVOKE ALL ON FUNCTION sm.transition_instance(text, uuid, text, text, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION sm.transition_instance(text, uuid, text, text, bigint) TO wicket_app;
