...
use wicket_core::{Identifier, PermissionKey, SignatureRequirement};
...
pub struct Edge {
    ...
    pub name: String,
    /// RBAC key checked by the executor via `wicket_identity::rbac::has_permission`.
...
/// [`wicket_db::Tx::begin`]. `wicket-db` does not expose `WriteContext::with_action`
/// or a session-GUC rebind; the executor verifies the bound action via `Tx::setting`.
pub fn with_action(
    mut ctx: wicket_db::WriteContext,
    doc: &DocRef,
    edge: &str,
) -> wicket_db::WriteContext {
