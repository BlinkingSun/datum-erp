...
impl Engine {
    ...
    pub async fn spawn(&self, tx: &mut Tx<'_>, doc: &DocRef, initial: &str) -> Result<Instance> {
        ...
        Ok(Instance {
            doc_type: doc.doc_type.clone(),
            doc_id: doc.doc_id,
            machine_id: MachineId(wicket_core::Identifier::from_uuid(id)),
            state: State(state),
            version,
            entered_at,
        })
    ...
    pub async fn transition(
        ...
        let bound = tx.setting("wicket.action").await?;
        if bound != expected {
        ...
        // (a) permission
        let allowed =
            wicket_identity::rbac::has_permission(tx, ctx.actor, &edge.permission.0).await?;
        if !allowed {
...
async fn load_machine_id(tx: &mut Tx<'_>, doc_type: &str) -> Result<Option<MachineId>> {
    let row: Option<(Uuid,)> = tx
        ...
        .await?;
    Ok(row.map(|(id,)| MachineId(wicket_core::Identifier::from_uuid(id))))
}
...
async fn load_catalog(
    ...
    let machine_id = MachineId(wicket_core::Identifier::from_uuid(id));
...
async fn load_instance(tx: &mut Tx<'_>, doc: &DocRef) -> Result<Instance> {
    ...
    Ok(Instance {
        doc_type: doc.doc_type.clone(),
        doc_id: doc.doc_id,
        machine_id: MachineId(wicket_core::Identifier::from_uuid(machine_id)),
        state: State(state),
        version,
        entered_at,
    })
