...
use wicket_core::{AnyQuantity, DimensionKind};
...
/// D-2b-13: one published order through `wicket-statemachine`.
pub async fn migrate_and_install(db: &wicket_test::TestDb) {
    let boot = db.bootstrap_pool().await.expect("bootstrap pool");
    wicket_module::order::install_upto(db.migrate_pool(), &boot, "wicket-statemachine")
        .await
        .unwrap_or_else(|e| panic!("install_upto wicket-statemachine: {e:#}"));
    boot.close().await;
...
pub fn write_pool_migrate(db: &wicket_test::TestDb) -> WritePool {
...
pub async fn persist_spawn(
    ...
    ctx: &WriteContext,
    doc: &DocRef,
) -> wicket_statemachine::Instance {
    let mut tx = wicket_db::Tx::begin(write, ctx).await.expect("begin");
