...
use wicket_statemachine::MIGRATOR;
...
async fn migration_is_reversible() {
    ...
    let app_exec: bool = sql_query_scalar(
        "SELECT has_function_privilege('wicket_app', 'sm.current_state(text, uuid)', 'EXECUTE')",
    )
    ...
    let spawn_exec: bool = sql_query_scalar(
        "SELECT has_function_privilege('wicket_app', 'sm.spawn_instance(text, uuid, uuid, text)', 'EXECUTE')",
    )
    .fetch_one(db.migrate_pool())
    .await
    ...
    assert!(
        spawn_exec,
        "EXECUTE granted to wicket_app on spawn_instance"
    );
    let app_update: bool =
        sql_query_scalar("SELECT has_table_privilege('wicket_app', 'sm.instance', 'UPDATE')")
            .fetch_one(db.migrate_pool())
            .await
            .expect("app update");
    assert!(!app_update, "UPDATE on sm.instance revoked from wicket_app");
    let app_insert: bool =
        sql_query_scalar("SELECT has_table_privilege('wicket_app', 'sm.instance', 'INSERT')")
            .fetch_one(db.migrate_pool())
            .await
            .expect("app insert");
    assert!(!app_insert, "INSERT on sm.instance revoked from wicket_app");
    let app_select: bool =
        sql_query_scalar("SELECT has_table_privilege('wicket_app', 'sm.instance', 'SELECT')")
            .fetch_one(db.migrate_pool())
            .await
            .expect("app select");
    ...
    assert_eq!(owner_after, "wicket_owner");
...
async fn catalogue_accepts_sm_schema() {
    ...
    wicket_db::ddl::check(db.migrate_pool())
        ...
        .expect("ddl check");
    let class: String =
        sql_query_scalar("SELECT class FROM wicket.schema_class WHERE nspname = 'sm'")
            .fetch_one(db.app_pool())
            .await
            .expect("class");
    assert_eq!(class, "app");
