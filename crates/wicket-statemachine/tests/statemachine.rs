...
use wicket_core::{NoPostings, NoSignatures, PostingError, PostingSink};
use wicket_db::Tx;
...
async fn engine_ready(required: bool) -> (Engine, DocRef) {
    ...
    let doc = DocRef {
        doc_type: "wo".into(),
        doc_id: wicket_core::Identifier::generate(),
    };
    (eng, doc)
}
...
async fn required_edge_refuses_without_token() {
    ...
    assert!(
        matches!(
            err,
            Error::Signature(wicket_core::SignatureError::Invalid(_))
        ),
        "got {err:?}"
    );
    tx.rollback().await.expect("rollback");
