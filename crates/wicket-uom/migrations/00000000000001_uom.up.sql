...
SELECT
  ...
  pg_catalog.set_config('wicket.action',        'uom.migrate', true),
  pg_catalog.set_config('wicket.source_kind',   'migration', true);

CREATE EXTENSION IF NOT EXISTS btree_gist;

CREATE SCHEMA IF NOT EXISTS uom AUTHORIZATION wicket_migrate;

REVOKE ALL ON SCHEMA uom FROM PUBLIC;
GRANT USAGE ON SCHEMA uom TO wicket_app;
GRANT USAGE, CREATE ON SCHEMA uom TO wicket_migrate, wicket_owner;

INSERT INTO wicket.schema_class (nspname, class) VALUES ('uom', 'app')
ON CONFLICT (nspname) DO UPDATE SET class = EXCLUDED.class;

ALTER DEFAULT PRIVILEGES FOR ROLE wicket_migrate IN SCHEMA uom
  GRANT SELECT, INSERT, UPDATE ON TABLES TO wicket_app;
ALTER DEFAULT PRIVILEGES FOR ROLE wicket_migrate IN SCHEMA uom
  GRANT TRIGGER ON TABLES TO wicket_owner;
ALTER DEFAULT PRIVILEGES FOR ROLE wicket_owner IN SCHEMA uom
  GRANT SELECT, INSERT, UPDATE ON TABLES TO wicket_app;
ALTER DEFAULT PRIVILEGES FOR ROLE wicket_migrate IN SCHEMA uom
...
ALTER TABLE uom.unit OWNER TO wicket_owner;
...
ALTER TABLE uom.item_stock OWNER TO wicket_owner;
...
ALTER TABLE uom.factor OWNER TO wicket_owner;
...
ALTER TABLE uom.rounding_policy OWNER TO wicket_owner;
...
END
$uom$;
ALTER FUNCTION uom.item_has_postings(uuid) OWNER TO wicket_owner;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'wicket: item stock measure immutable while postings exist'
      USING ERRCODE = 'check_violation';
  END IF;
  RETURN NEW;
END
$uom$;
ALTER FUNCTION uom.enforce_stock_item_immutable() OWNER TO wicket_owner;
...
GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA uom TO wicket_app;
REVOKE DELETE ON ALL TABLES IN SCHEMA uom FROM PUBLIC, wicket_app;

GRANT EXECUTE ON FUNCTION uom.item_has_postings(uuid) TO wicket_owner, wicket_migrate;
