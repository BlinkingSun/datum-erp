...
pub struct UomCatalog {
    ...
    policies: HashMap<(ItemId, UnitId, Operation), wicket_core::Rounding>,
    as_of: DateTime<Utc>,
}
...
impl UomCatalog {
    ...
    pub fn rounding_rule(
        ...
        unit: UnitId,
        op: Operation,
        default: wicket_core::Rounding,
    ) -> wicket_core::Rounding {
...
pub async fn load_catalog(tx: &mut Tx<'_>) -> Result<UomCatalog> {
    ...
    for (id, dim, scale) in unit_rows {
        let dimension = dimension_from_db(&dim).ok_or_else(|| {
            crate::Error::Core(wicket_core::Error::Invariant(format!(
