...
//!
//! `wicket-uom` cannot depend on `wicket-ledger` (CONTRACT §4 dependency graph:
//! `wicket-ledger` → `wicket-uom`). R5 postings presence uses the ledger query
//! seam `ledger.has_postings` on the caller's [`Tx`] (same source of truth as
//! `wicket_ledger::has_postings`).
