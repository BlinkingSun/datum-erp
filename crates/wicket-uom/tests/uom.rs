//! Integration tests for `wicket-uom` (Postgres when `WICKET_REQUIRE_PG=1`).
#![allow(missing_docs)]
...
async fn effectivity_picks_the_factor_in_force() {
    let db = wicket_test::db_case!("effectivity");
    common::migrate(&db).await;
    let pool = WritePool::new(db.app_pool().clone());
...
async fn inverse_factor_is_inferred_exactly() {
    let db = wicket_test::db_case!("inverse_factor");
    common::migrate(&db).await;
    let pool = WritePool::new(db.app_pool().clone());
...
async fn repin_closes_the_open_pin() {
    let db = wicket_test::db_case!("repin");
    common::migrate(&db).await;
    let pool = WritePool::new(db.app_pool().clone());
...
async fn lot_factor_beats_item_factor_beats_global() {
    let db = wicket_test::db_case!("factor_rank");
    common::migrate(&db).await;
    let pool = WritePool::new(db.app_pool().clone());
...
async fn pin_item_stock_inserts_canonical_measure() {
    let db = wicket_test::db_case!("pin_item_stock");
    common::migrate(&db).await;
    let pool = WritePool::new(db.app_pool().clone());
...
async fn pin_then_to_stock_in_one_tx() {
    let db = wicket_test::db_case!("pin_to_stock");
    common::migrate(&db).await;
    let pool = WritePool::new(db.app_pool().clone());
...
async fn to_stock_with_and_without_lot_pin() {
    let db = wicket_test::db_case!("to_stock_lot");
    common::migrate(&db).await;
    let pool = WritePool::new(db.app_pool().clone());
...
async fn seeded_units_have_right_dimensions() {
    let db = wicket_test::db_case!("seed_dims");
    common::migrate(&db).await;
    let pool = WritePool::new(db.app_pool().clone());
...
async fn stock_unit_immutable_while_postings_exist() {
    let db = wicket_test::db_case!("stock_immut");
    ...
    assert!(
        wicket_ledger::has_postings(&mut tx, item).await.unwrap(),
        "wicket_ledger::has_postings must see the posting before update_item_stock refuses"
    );
...
async fn every_uom_table_is_audited() {
    let db = wicket_test::db_case!("audited");
    common::migrate(&db).await;
...
async fn round_trip_ab_a_within_one_ulp_at_stock_scale_8() {
    let db = wicket_test::db_case!("roundtrip_s8");
    common::migrate(&db).await;
    let pool = WritePool::new(db.app_pool().clone());
...
fn round_half_even_at_stock_scale() {
    ...
    for (amount, scale, want_value, want_residual) in ties {
        let converted = wicket_core::Converted::new(amount, unit, scale);
...
proptest! {
    ...
    fn residual_is_never_dropped(unscaled in 1i64..100_000i64) {
        ...
        let converted = wicket_core::Converted::new(exact, unit, 4);
        let (value, residual) = apply_rounding_policy(converted, Rounding::HalfEven);
