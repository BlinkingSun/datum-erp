...
costs, and which Datum claims are present facts versus unbuilt phases. A contributor
...
Datum is trying to be that product as open source, with audit and signature as kernel
...
LGPL Community is more adoption-friendly than Datum's AGPL-3.0-or-later
...
| Product | Price signal | Grade | Why a shop looks, and why it is not Datum's wedge by itself |
|---|---|---|---|
...
quoting tool and spreadsheets across the seam. Datum's durable difference is a
...
- An inventory ledger from which quantity is derived. ERPNext has one
  (`research/audits/slice-competitive.md` §2.1). Datum's append-only, zero-sum groups
...
## 6. What Datum should not compete on
...
to seed an estimate inside Datum; the buying-criterion job is already served.
Integrate first.
