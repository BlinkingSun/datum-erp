...
- `sqlx::query` / `query_as` / `query_scalar` are allowed in this crate (the sealed `Tx` helpers take sqlx query
  ...
  `QueryBuilder`, the `query!` macros) stays confined to `datum-db` / `datum-audit` / `datum-test`.
- **Named test `writes_go_through_tx`**: a write to one of this crate's audited tables on a raw pool connection
  ...
  legitimate; every write path in the crate goes through `datum_db::Tx`.

## ADDENDUM — fence is a law, not a grep (2026-09-12 05:40)

CONTRACT §5a.1 binds this crate: no `allow(clippy::disallowed_*)` at any scope, no `sqlx` token in any `build.rs`, no generated query code, no runtime DDL/GRANT/CREATE EXTENSION from Rust (all DDL in migrations run as `datum_migrate`). Plain `sqlx::query`/`query_as`/`query_scalar` through the sealed `datum_db::Tx` are the only SQL entry points; `just clippy` and `just lint-sql` must be green with the fence exactly as landed on main. Circumvention is a Fail-class finding (measured on datum-uom attempt 1).

## ADDENDUM — kernel seams every slice module must respect (2026-09-12 10:56, from the batch 2.6 glue adjudication)

- Hooks run under the sealed, synchronous `HookView` ABI: a hook cannot call `datum_uom` `to_stock`/`convert`
...
- **R-2s-2 one name triple.** Crate `datum-mod-<x>` (Cargo `[package] name`), manifest id `mod-<x>` (`module.toml` `id`, `profiles/*.toml`, `datum-module` compiled-in list), schema `<x>`. 2s.1 landed with three different conventions (items/items/items, datum-mod-locations/mod-locations/locations, lots/lots/lots); the 2s1-glue lane aligns them to the triple. Every later module ships aligned.
...
- **R-2s-8 SECURITY DEFINER exemption set (2026-09-12 18:45, from lint-migrations findings).** `CREATE FUNCTION … SECURITY DEFINER` is allowed only in the migrations of `datum-db`, `datum-audit`, `datum-ledger` and `datum-numbering` (gap-free counter and exemption helpers, D3 §8), each documented in that crate's README. No module and no other kernel crate may define one; a cross-schema read or GRANT inside such a function is a fence bypass. `uom.item_has_postings` and `items.item_has_postings` are removed in favour of `datum_ledger::has_postings` (R-2s-3). `just lint-sql` scans migrations as well as sources.
