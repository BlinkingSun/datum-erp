...
use datum_ledger::{children_of, CostMethod};
use datum_mod_inventory::{receive, release_from_quarantine, ReceiveRequest, ReleaseRequest};
...
pub async fn migrate_all(db: &datum_test::TestDb) {
    install_through(db, "datum-mod-inventory").await;
}
