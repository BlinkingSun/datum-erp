# PLAN AUDIT — Datum ERP foundation build
...
### R2. `Quantity<U: Unit>` poisons `datum-core`
...
- First demanded **edge:** `datum-documents → datum-statemachine` (approval workflow **is** a state machine). Also `documents → numbering`.
- First demanded **cycle:** `statemachine → ledger` (hooks contribute postings) **and** `ledger → statemachine` (posting marks the source document Posted). Invert with `PostingSink` in `datum-core`. Events cannot substitute (async, different transaction, group cannot balance).
- `datum-ledger → datum-identity` is a **false alarm** if `posted_by: Actor` stays in core.
- `datum-module` depends on everyone → not a parallel Wave 2 lane.
...
- Quality first-class: ERPNext ships QC in **core**; Odoo Quality is **Enterprise-only**. Datum’s depth is Phase 4.
...
| G6 | No `datum-test` crate; no `dev/compose.yml` Postgres; no two `DATABASE_URL`s (app vs migrate) | Invariant 3 untestable; `query!` unpreparable |
| G7 | `datum-core` treated as a stub peer; it must be **complete** in Wave 1 after Quantity DECISION | Everything imports it |
| G8 | Missing `datum-print` (or explicit fold into `datum-documents`) | 11.50(b); Annex 11 §8; extension-point fork |
...
| G12 | `datum-documents` missing SM and numbering edges | Lane cannot implement approval workflow |
| G13 | No `PostingSink` in core | Hook ABI will create SM↔ledger cycle |
| G14 | `datum-module` placed as parallel Wave 2 | Cannot honestly register real ABIs against stubs |
| G15 | Hook ABI (veto schema, posting envelope, order, budget) unstated | Wave 3 training/calibration veto will invent traits |
| G16 | `datum-numbering` algorithm unstated; SEQUENCE hole | Historical gaps are permanent audit questions |
| G17 | `datum-identity` 11.200/11.300 scope unstated (aging, lockout, two components, admin reset) | Schema now or a finding later |
...
|---|---|---|
| **D1 Quantity** | Dimension phantom + runtime `UnitId` + separate `Money`; not `Quantity<U: Unit>` | `datum-core`, every crate |
| **D2 Ledger slice** | Inventory `(group, item, uom)`; lot/serial **not** conservation keys; cost/labor per ledger; canonical stocking UoM before insert; `posting_group` header for empty-group | `datum-ledger` trigger + proptest ACs |
| **D3 Audit persistence** | Trigger + fail-closed GUC + sealed `Tx` + lint; **revoke INSERT** on audit from app role (`SECURITY DEFINER` only); time = `clock_timestamp()` or freeze `now()` with one-group-per-txn | `datum-db`, `datum-audit`, Wave 1 roles |
| **D4 Tamper-evidence** | Grant+trigger for v1 app-bug net; hash chain **if** marketing keeps “cannot obscure,” with off-box verify; **do not** pgcrypto with key in PGDATA | `datum-audit` row type |
...
| **D6 Print** | Add `datum-print` to §5 **or** fold into `datum-documents` with esign edge, **and** sixth extension point (template packs) | Wave 1 member list; 11.50(b) |
| **D7 License** | Close ADR 0006 before `LICENSE` / `Cargo.toml license` | `doc-repo`, workspace |
| **D8 Conservative 11.200** | Always all identification components (ignore continuous-session relaxation) until a shop asks | `datum-esign` |
...
|---|---|---|---|
| `datum-core` | **cursor**; **serial batch 1**; **not a stub** | one crate | **deep + double** |
| `datum-db` | **cursor** | serial batch 2 after core | **deep** (PLAN unspecified — **differ**) |
| `datum-audit` | **cursor** | serial after db | **deep + double** |
| `datum-identity` | **cursor** | after audit; authn/RBAC/principals one crate | **deep** |
| `datum-esign` | **cursor** | serial after identity | **deep** |
| `datum-uom` | **cursor** | after core D1; ∥ identity | **deep** |
| `datum-numbering` | **grok** | after core+db; **no SEQUENCE** | **deep** for gap-free (PLAN would be standard — **differ**) |
| `datum-events` | grok | after core+db | standard |
| `datum-jobs` | grok | after events; **no identity edge** (`Actor` at construct) | standard |
| `datum-customfields` | grok | after audit | standard |
| `datum-ledger` **engine** | **cursor**; **blind-race after D2 freeze** | `src/` + migrations; serial after uom+audit | **deep + double** |
| `datum-ledger` **proptest** | **grok** (other family) | `tests/` or `datum-ledger-proptest`; **split from engine** (protocol — **differ**) | **deep + double** |
| `datum-statemachine` | **cursor** | after identity+esign; `PostingSink` not ledger types | **deep** |
| `datum-documents` | **cursor** | **serial after SM + numbering** (new edges) | **deep** (PLAN would treat as standard — **differ**) |
| `datum-print` | grok | **add**; after documents / with documents | **deep** once added |
| `datum-module` | **cursor** | **serial Wave 2.5** (or thin registry ∥ with deps `core db audit` only) | **deep** |
| `datum-server` | cursor | Wave 3 composition root | **deep** |

**Do not add:** `ledger → identity`, `statemachine → ledger`, `jobs → identity`, `uom → items`, `datum-gl`.  
...
| `cargo test -p datum-ledger` default | **fast set only** (<2 min target): in-memory 256 cases + 64 PG cases P1/P2/P5/P6. See sweep-plan-proptests.md §7.4 |
...
1. §5: replace “`Quantity` carrying its unit of measure as a type parameter” with dimension+`UnitId` (D1). Add `PostingSink`. Add `datum-test`. Add `datum-print` or fold. Name every Wave 2 lane. Add documents→SM, documents→numbering.
...
5. §3: Wave 2 is compile-parallel, done in batches; `datum-module` is 2.5; Phase 1 modules are a named gate of **this** build (Wave 3), not “kernel only” as scope.
