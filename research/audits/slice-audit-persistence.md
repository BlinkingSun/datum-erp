...
**The PLAN is missing a mechanical enforcement lane.** Wave 1 `workspace` must ship roles, grants, two connection URLs, the session-context helper, the audit-attach event trigger, and a lint that is allowed to fail the build. Wave 2 `datum-db` / `datum-audit` cannot invent this per crate.
...
AS $$
DECLARE
  actor text := current_setting('datum.actor_id', true);
  reason text := current_setting('datum.reason', true);
  source_type text := current_setting('datum.source_type', true);
  source_id text := current_setting('datum.source_id', true);
  action text := current_setting('datum.action', true);
BEGIN
  IF actor IS NULL OR actor = '' THEN
    RAISE EXCEPTION 'datum: write without actor (SET LOCAL datum.actor_id)'
      USING ERRCODE = '42501';
  END IF;
...
Non-audited tables (ADR 0005 last cost bullet) are a **catalog decision**, not a per-write flag: e.g. `audit.exempt(schema, table, reason, decided_by)` written by `datum_migrate`, itself audited as DDL. An event trigger that skips `audit.exempt` is the implementation of “declaration is a reviewed, recorded decision.”

**This is the structural half.** It does not by itself supply business intent. That is the GUC protocol in §3.

**“Cannot write a false one” requires a grant the ADR currently refuses to make.** If `datum_app` has `INSERT` on `audit.event`, any module can insert a fake row. The trigger function must be `SECURITY DEFINER`, owned by a NOLOGIN owner role, and `datum_app` must have **SELECT only** on `audit.event`. The trigger owner has `INSERT`. That is the only way “cannot write a false one” is a grant, not a vibe.
...
### (b) Sealed `Write` trait in `datum-db` + lint forbidding `sqlx::query` in module crates

A repository/UoW you write yourselves:

```rust
// datum-db, public, sealed
...
pub trait Write: Sealed { /* not implemented outside datum-db */ }
```
...
| Cannot write a false audit row | `datum_app` has **no INSERT** on `audit.event`; only `SECURITY DEFINER` trigger function inserts |
...
| Attributable actor | Fail-closed `current_setting('datum.actor_id', true)` |
| Business intent (reason/source/action) | GUC / SET LOCAL, set by `datum_db::Tx::begin`; required per-table where the catalog says so |
| Modules cannot bypass in-app | CI lint + `disallowed_methods`/`disallowed_macros` on `sqlx::query*` outside `datum-db`/`datum-audit` + deny `copy_in_raw` in module crates |
...
Trigger-only, as the ADR already says, is “partially adopted” because OLD/NEW is not intent. That is correct. The missing half is not a SQLx interceptor. It is a **fail-closed session-context protocol** plus a helper that is the only way `datum_app` is allowed to start a write transaction.
...
Custom GUCs with a dotted name need **no** `postgresql.conf` entry since PG 9.2 (`custom_variable_classes` is gone). `SET datum.actor_id = '…'` just works as a placeholder GUC.
...
-- SET LOCAL datum.actor_id = $1;

-- Valid, and what SQLx must emit:
SELECT set_config('datum.actor_id', $1, true);
SELECT set_config('datum.actor_kind', $2, true);     -- 'user' | 'service'
SELECT set_config('datum.reason', $3, true);
SELECT set_config('datum.source_type', $4, true);
SELECT set_config('datum.source_id', $5, true);
SELECT set_config('datum.action', $6, true);
SELECT set_config('datum.request_id', $7, true);
```

`datum-db` owns this. `Tx::begin(pool, ctx: WriteContext) -> Tx` runs `BEGIN` then the `set_config` batch. There is no `pool.begin()` in module crates.
...
|---|---|---|
| **Connection-pool GUC leakage** | `set_config(..., false)` or `SET datum.actor_id` (session) survives `COMMIT` and is still set when SQLx returns the conn to the pool. Next HTTP request on that conn audits as the previous operator. SQLx does **not** `DISCARD ALL` on release. | Ban session-level SET for `datum.*`. Only `is_local=true` after `BEGIN`. Optional belt: `after_release` runs `DISCARD ALL` / `RESET ALL` — but `DISCARD ALL` cannot run inside a tx, so it is a release-path thing. `Tx::begin` must still SET LOCAL. |
...
| **Prepared statements** | `SET LOCAL name = $1` is a syntax error. People will try it. | Only `set_config`. Document in `datum-db`. `query!` handles `set_config` fine. |
...
| **COPY** | `COPY FROM` fires row triggers (PG docs). It does **not** set GUCs. Fail-closed trigger aborts the COPY unless the session already has context. SQLx `copy_in_raw` is a second write path with no `query!`. | Allow COPY only on `datum_migrate` (seed/restore) or after `Tx::begin`. Lint-ban `copy_in_raw` in module crates. |
| **TRUNCATE** | Row triggers do not fire. Statement-level `AFTER TRUNCATE` must exist or truncates are silent. `TRUNCATE` is also a separate privilege. | Always attach a TRUNCATE trigger. Do not `GRANT TRUNCATE` to `datum_app`. |
| **Migrations** | `sqlx::migrate!` executes as the connected user. `datum_app` cannot `CREATE TABLE`. If migrations run as superuser, they also skip the two-role story unless the last statements `GRANT`/`REASSIGN`. Event trigger on `CREATE TABLE` will try to attach audit triggers during migrations — good, unless the migrate role cannot insert into `audit.event` and the event trigger is not `SECURITY DEFINER`. | Wave 1: `DATABASE_URL` for migrate ≠ app. Event-trigger function `SECURITY DEFINER`. |
| **`session_replication_role = replica`** | User triggers **do not fire**. Superuser-only to SET by default (context = superuser). `datum_app` must not have `SET` on this GUC. Owner/superuser can. This is the standard “bypass audit triggers” switch. | `REVOKE SET ON PARAMETER session_replication_role FROM datum_app` (PG 15+ parameter ACLs). Still does not bind the owner. |
| **`ALTER TABLE … DISABLE TRIGGER`** | Table owner can disable the audit trigger. Then writes are silent. | Owner is `datum_owner` NOLOGIN. App and migrate are not owners. Still does not bind superuser. |
...
| **GUC is text, attacker-settable** | `datum_app` can `set_config('datum.actor_id', 'someone-else', true)` and impersonate. The trigger trusts the GUC. | The sealed `Tx::begin` takes a kernel `Actor` already authenticated by `datum-identity`, not a string from the handler. Lint so modules cannot call `set_config` themselves (`disallowed_macros` / grep `set_config` / `current_setting`). This is house style on top of (a). Without it, “cannot write a false one” fails for **actor**, even if the row image is true. |
...
| **Background jobs** | ADR 0005: named service principal. A job that opens a pool connection and writes without `Tx::begin` dies fail-closed. | `datum-jobs` must go through `Tx::begin(Actor::service("datum-jobs"))`. PLAN crate graph does not give `datum-jobs` an `audit` or `identity` edge (only `core db events`). **That is a demanded edge or a `Actor` in `datum-core` plus `Tx` in `datum-db`.** Flag for slice 3. |
...
Transaction-local session **table** (`CREATE TEMP TABLE datum_tx_ctx ON COMMIT DROP`) is the alternative to GUC. Temp tables in a pool leak if not `ON COMMIT DROP`; they also need `INSERT` that can itself recurse if that table is audited. **GUC SET LOCAL is the less wrong tool.** Do not do both.
...
| Role | Login | Owns | May |
|---|---|---|---|
| `datum_owner` | NOLOGIN | all tables, trigger functions | nothing at runtime; `REASSIGN OWNED` target |
| `datum_migrate` | LOGIN | member of a migrate group; `SET ROLE datum_owner` for DDL | `CREATE`/`ALTER`/`GRANT`; **not** the app pool URL |
| `datum_app` | LOGIN | nothing | DML as granted: `SELECT, INSERT, UPDATE, DELETE` on business tables; **`SELECT` only** on `audit.event`; `USAGE` on sequences; **no** `TRUNCATE`, **no** `TRIGGER`, **no** `SET session_replication_role` |
| `datum_audit_writer` | NOLOGIN | `audit.row_change` | `INSERT` on `audit.event` (used only via `SECURITY DEFINER`) |

`sqlx::migrate!` **must** use `postgres://datum_migrate@…`. The app pool **must** use `postgres://datum_app@…`. Two env vars. Wave 1 `dev/` and CI compose/testcontainers must create both.

DDL vs append-only: **workable**. Migrations never connect as `datum_app`. The app role never needs DDL. The tension the question poses (“grants vs migrations”) is only a problem if a single role does both. That is the PLAN gap.

Initial migration (owned by `workspace` or `datum-db`, not by a Wave 2 afterthought):

```sql
CREATE ROLE datum_owner NOLOGIN;
CREATE ROLE datum_audit_writer NOLOGIN;
...
ALTER TABLE audit.event OWNER TO datum_owner;
REVOKE ALL ON audit.event FROM PUBLIC;
GRANT SELECT ON audit.event TO datum_app;
GRANT INSERT ON audit.event TO datum_audit_writer;
REVOKE UPDATE, DELETE, TRUNCATE, TRIGGER ON audit.event FROM datum_app;
```
...
`session_replication_role` is superuser-context. Parameter ACLs (PG 15+) can stop `datum_app`. They cannot stop the cluster owner.

### Hash chain / pgcrypto / “even the owner”

| Control | Stops `datum_app` | Stops owner `psql` | Stops restore-from-backup | Stops “edit PGDATA on disk” |
|---|---|---|---|---|
...
1. Grant-level append-only + trigger-block + `datum_app` has no INSERT on audit = **necessary**, and it is what PLAN invariant 3 should mean.
...
4. The bundled-PG reality (slice 7) and this slice are the same threat. If opus decides “escape-hatch Postgres, customer’s DBA,” the owner-threat is the customer’s SOP problem. If opus decides “we bundle and we are the DBA,” Datum owns the 11.10(e) story for the superuser path.
...
Wrong instinct: a `datum_test` role with `DELETE` on `audit.event` in the same cluster. That role will leak into fixtures, CI, and eventually a “reset demo” button.

Right instincts, in order:

1. **Ephemeral database per test.** `sqlx::test` (0.7+) / testcontainers / a Wave 1 helper that `CREATE DATABASE … TEMPLATE datum_template`. The test DB is dropped. No DELETE. This is the only strategy that also exercises deferred ledger constraints and concurrent tests.
...
3. **Migrate-role reset of a dedicated `datum_test` database**, never of a data dir that could be production. `DROP SCHEMA … CASCADE` as owner. Documented, gated on `current_database() IN ('datum_test', …)` inside a SQL function owned by `datum_migrate`. Still a loaded gun; prefer (1).

Do **not** `TRUNCATE audit.event` as `datum_app`. Do **not** grant it. A test that needs a clean audit table is in the wrong database.
...
Wave 1 must ship this helper (`datum_db::test_db` or a `dev/test-db.sh`). PLAN §7 does not mention it. Slice 4 (stubs) will collide with this if `sqlx::test` needs a live Postgres before the installer exists — same gap as slice 7.
...
| No session-context helper | Every Wave 2 crate will `pool.begin()` and forget SET LOCAL | Wave 1 stub of `datum-db`: `Tx::begin(Actor)` is **real**, not `todo!()` |
| No fail-closed trigger SQL | “Produced by the persistence layer” has no artifact | Wave 1 or serial start of `datum-audit`: `migrations/0001_audit.sql` |
...
| No lint forbidding raw SQLx writes outside `datum-db` | House style is otherwise unenforceable | Wave 1: `clippy.toml` `disallowed-macros` / `disallowed-methods` + CI `rg` for `sqlx::query`, `query!`, `QueryBuilder`, `raw_sql`, `copy_in_raw`, `set_config` |
...
| `datum-jobs` has no audit/identity edge | Background work must SET LOCAL a service principal | crate-graph slice; `Actor` in `datum-core` + `Tx` in `datum-db` may save the edge |
| SQLx version unpinned | 0.7 vs 0.8 vs 0.9 changes `Executor` and `SqlStr` | Wave 1 `Cargo.toml` workspace.dependencies |
| Non-audited table protocol unspecified | ADR allows it; without a catalog it becomes a per-write flag | `datum-audit` SPEC |
| Test DB strategy unspecified | ADR admits the pain and then PLAN says nothing | Wave 1 `dev/` + `#[sqlx::test]` example in stub |
| Auto-audit of DDL (who ran a migration) | Event trigger on `ddl_command_end` writing to `audit.ddl` | `datum-audit`; not invariant 3 but 11.10(k) adjacent |

**The missing mechanical enforcement lane, named:** Wave 1 ships (1) roles+grants, (2) `Tx::begin` helper, (3) audit trigger + event trigger SQL, (4) clippy/CI deny list. Wave 2 `datum-audit` owns the schema and tests that a `query!` insert without `Tx::begin` **fails**, that `datum_app` `UPDATE audit.event` **fails**, and that a forged `INSERT INTO audit.event` **fails**. That test list is the acceptance criterion PLAN §6 invariant 3 currently lacks.
...
| `datum-db` `Tx::begin` / `WriteContext` / pool `after_connect` UTC | Wave 2 crate lane | do not split the helper from the pool | **deep** |
| `datum-audit` schema, triggers, event trigger, exempt catalog, tests | Wave 2 crate lane, **serial after** `datum-db` stub is real enough to compile tests | tests **do** split: `audit-engine` vs `audit-pg-tests` if the PG suite > 10 min | **deep** |
| Hash chain / signed export (if DECISION says yes) | same `datum-audit` lane or a follow-on; **not** a module | keep in kernel | **deep** |
...
`datum-audit` is integration-critical and previously-unspecified → PLAN already implies deep by putting it on the kernel path; this slice **agrees, and raises `datum-db` to the same tier.**
...
2. **Audit INSERT grant:** revoke from `datum_app`, `SECURITY DEFINER` only (**recommended**), or keep ADR 0005 as written and drop “cannot write a false one”?
...
Until that file exists, Wave 1 may still ship roles and a fail-closed trigger (those do not pre-empt 4). It must **not** grant `datum_app` INSERT on `audit.event` “to match the ADR” — that cements the contradiction.
...
Yes, if and only if migrate and app are different roles. PLAN never says that. It is a missing Wave 1 subtask. Grant-level append-only is then workable against `datum_app` and is not workable against the bundled-cluster owner. Do not pretend otherwise; put the owner threat in the IQ/SOP story and optionally a verifiable export.
