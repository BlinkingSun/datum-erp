...
### 2.1 Stock Ledger Entry is a ledger. It is not Datum’s ledger.
...
Cannot enable both batch and serial on one item in older versions; v15+ serial-and-batch bundle relaxes this. Genealogy as a **graph of what went into what, including operator, gage, inspection** is not a core object. Datum catalog `genealogy` + `dhr` is the actual gap.
...
Core ERPNext does **not** claim Part 11. Partner apps do (SigzenPHARMA “21 CFR Part 11 Ready”). Treat those as unverified marketing until a 510(k) holder’s validation package is public. Datum should not compete with that sentence; it should compete with “audit produced by the persistence layer, e-sign bound to record hash, grant-level append-only.”
...
| Capability | Community 19 (2025–26) | Enterprise 19 | Implication for Datum |
|---|---|---|---|
...
| Audit log | **No** in core. OCA `auditlog` (create/read/write/delete rules, not grant-level, not automatic) | Better field tracking in EE marketing; still not kernel | Matches Datum’s “add-on trail is how you get a partial trail” critique **for CE**. EE is better and still not 11.10(e). |
| License | **LGPLv3** | Proprietary OEEL + per-user | CE is *more* permissively licensed than Datum’s AGPL recommendation |
...
- Datum’s success test 6 (“never encounters a quality screen”) is only a wedge if **kernel audit/e-sign remain on while quality modules are off**, and the UI actually suppresses regulated navigation. That is a product requirement, not a slogan. Waves 1–2 cannot demonstrate it.
...
| Product | Why they look | Why they lose / win vs Datum-as- pitched | Price signal (2026, third-party) |
|---|---|---|---|
| **ProShop** | **Closest competitor.** ERP+MES+QMS, paperless, machine-shop native, claims ISO 13485 + 21 CFR 11, document control, NCR, training, AS9100. Built on a shop floor. | Closed, per-employee pricing, not OSS, vendor holds the records. **This is the product Datum is trying to be, as open source.** If `docs/08` does not put ProShop in paragraph 1, it is the wrong document. | Quote; third-party ~$500–$1,800/mo for 5–30 heads ([Softwarefinder 2026-09](https://softwarefinder.com/enterprise-resource-planning-software/proshop-erp/pricing)); priced on **headcount not seats** |
...
| **Greenlight Guru** | Med-device-only, design controls + QMS | ~$25k–$60k/yr; $600/user/mo anecdotes; 2026 package-separation price shock | High for **design-control** shops. Weak for **contract manufacturers** who do not own the DHF. Datum correctly defers DHF to PLM. |
...
| Product | What it actually does | Relationship to Datum `cad` module |
|---|---|---|
| **Paperless Parts** | Ingests CAD (including assemblies), extracts geometry and BOM tree, costs operations, flags unmanufacturable features, medical-device routing templates (FAI, sampling, traceability). **Not an ERP.** Pushes into JobBOSS / ProShop / GSS / Epicor / Infor. | **Owns the “CAD to estimate” job.** Datum Phase 7 would be a late, in-ERP clone of a product shops already buy. |
...
| PLAN §3 Wave 3 | Covers `datum-server`, **Phase 1 modules**, web shell, Tauri | Phase 1 **is** in this build |
