...
needs administrator rights. Both true. **`wicket-server` has exactly the same problem.** A Rust binary
...
nobody logged in — and that requires `wicket-server` to be a Windows Service whether the database is
...
Once one elevation prompt is accepted for Wicket's own service, the marginal cost of PostgreSQL also
...
> **Wicket never owns a database process lifecycle. On any operating system. In any version.**
> The operating system's own service manager owns the daemon — Windows Service, launchd, systemd.
> Wicket owns everything above the socket.

Everything above the socket is: probing for a reachable cluster, creating the database, creating the
`wicket_app` and `wicket_migrate` roles, installing extensions, applying the grants that make the audit
...
is, or whether you broke something. Wicket's first-run wizard kills all five: it probes for a cluster,
...
> 1. **Install.** On a machine that already runs PostgreSQL 16 or later, a shop installs Wicket and is
...
>    told exactly which version, which download, and what to click. Wicket's first-run wizard does every
...
**Tier 1 — `wicket demo`.** A single signed executable per operating system. It unpacks a PostgreSQL via
...
2. There is no promotion path. `wicket restore` and `wicket migrate` refuse a demo data directory
...
**Tier 2 — `docker compose up`.** One YAML file, `postgres:17` plus `wicket-server`. For developers, for
...
**Tier 3 — production.** OS-managed PostgreSQL, Wicket as a native service. The only path supported for
...
installer as the `postgresql-x64-NN` Windows Service under its own service account; and Wicket installed
from a signed MSI that requires elevation exactly once, at install, and which registers `wicket-server`
as a Windows Service set to **Automatic (Delayed Start)**, creates one inbound firewall rule for the
HTTP port, and writes configuration to `%PROGRAMDATA%\Wicket`, not `%LOCALAPPDATA%`. Two elevation
...
- `wicket-server` connects to **`127.0.0.1`**, never `localhost`. Windows resolves `localhost`
...
- PostgreSQL stays loopback-only. Only `wicket-server` binds the LAN. One firewall rule, one process.
- Wicket never runs `initdb`, so the restricted-token trap, the codepage-versus-UTF8 trap, and the VC++
  redistributable trap all move to a vendor who has already handled them. Wicket instead *verifies*
...
wearing a different hat. Wicket installs from a signed `.pkg` whose postinstall lays down
`com.wicket.server.plist` as a LaunchDaemon and registers the binary with the application firewall.
...
`apt install postgresql-17` or the `dnf` equivalent, then Wicket's `.deb` or `.rpm`, whose postinst
creates the system user, installs `wicket.service` with `After=postgresql.service`, and installs
`wicket.timer` for the nightly backup. systemd is the native lifecycle and reinventing it inside an
...
**In the form ADR 0003 described — Wicket's own process tree initializing, starting, stopping and
...
*composite installer*: Wicket's own installer, already elevated for its own service, invokes the
platform's PostgreSQL installer unattended and then hands the resulting cluster to the operating
system's service manager and walks away. Wicket still never owns the lifecycle. This is what turns the
...
1. Wicket ships code-signed installers on all three operating systems from an established signing
...
> Wicket owns the mechanism, the verification, and the proof.**
...
**What Wicket ships in v1** — this closes plan-audit **G9**, which flagged backup as claimed in `docs/02`
and absent from every wave:

- **`wicket backup`** — `pg_dump --format=custom` of the Wicket database plus a manifest: schema version,
...
- **`wicket backup verify <file>`** — restores into a scratch database, re-runs the ledger conservation
...
- **`wicket restore <file>`** — into an empty database; creates roles and grants, restores, then runs
...
**What Wicket does not own:** where the copy goes, how many generations are kept, how long they are
...
An **ordinary shop** is done with: nightly `wicket backup` to a second physical disk or a NAS, weekly
automatic `verify`, and a restore they tried once. Wicket's defaults give them all three.
...
1. **A written retention period derived from their predicate rule**, not from a convenient default. For
   ...
   from commercial release. Wicket must therefore never prune anything on its own schedule, and in
   particular must never prune audit rows on a schedule that differs from the records they describe —
   11.10(e) forbids it. *Ours to enforce: Wicket has no data expiry feature and will not grow one.*
...
Wave 3 was `wicket-server`, the Phase 1 modules, the web shell, and a Tauri desktop shell. Bundling was
...
- **`wicket-server` as a native service on all three platforms**, with real installers: a signed MSI
...
- **`wicket demo`**, reusing the Wave 1 embedded-PostgreSQL test harness, plus the three refusals.
- **The `docker compose` evaluation file.**
- **`wicket backup` / `verify` / `restore` and the scheduled job**, closing G9.
- **`wicket iq`**, the customer-runnable installation qualification suite already promised in
  `docs/02` §9.
...
|---|---|---|---|---|
| a | Saturday evaluation, Windows 11 shop PC, no help | Download `wicket-demo-windows-x64.exe` (~90MB) → double-click → unpack, `initdb`, seed → browser opens on `127.0.0.1` | **3–5 min** | **none** |
| b | Thirty-person shop into production, small Windows server | EDB PostgreSQL 17 installer → Wicket MSI → first-run wizard → first admin user → backup job | **15–20 min** | **2**, both at install |
| c₁ | Linux box, Ubuntu 24.04 server | `apt install postgresql-17` → `apt install ./wicket.deb` → wizard | **6–8 min** | `sudo`, twice |
| c₂ | Mac mini in the office | `brew install postgresql@17` → `sudo brew services start` → `Wicket.pkg` → wizard | **10–15 min** (20–25 without Homebrew) | admin password, twice |
...
| f | PostgreSQL major upgrade, year two, validated install | Customer IT installs PG N+1 alongside → stop Wicket → vendor `pg_upgrade` → repoint → `verify` + IQ | **2–4 h window** | yes; it is IT's task |
| g | Machine dies, restore to new hardware, prove intact | Rebuild per (b) → `wicket restore` → automatic `verify` + `iq` → signed PDF report | **30–45 min**, proof in the same run | 2, per (b) |
...
It unpacks PostgreSQL into `%LOCALAPPDATA%\Wicket\demo`, runs `initdb` with an explicit `-E UTF8`, binds
...
3. Download `Wicket-x.y.z-x64.msi`. **1 min.**
4. Run it. **UAC prompt two.** It registers `wicket-server` as a Windows Service, Automatic (Delayed
   Start), creates one inbound firewall rule for TCP 8080, and writes configuration under
   `%PROGRAMDATA%\Wicket`. **1 min.**
5. Browser to `http://localhost:8080`. The wizard has already found the cluster on 5432. It asks for the
   superuser password once, uses it, and does not store it: it creates the `wicket` database, the
   `wicket_app` role with no `UPDATE` or `DELETE` on audit, the `wicket_migrate` role, installs
...
and enabled at boot. `sudo apt install ./wicket_x.y.z_amd64.deb` — **1 min**; the postinst creates the
`wicket` system user, installs `wicket.service` with `After=postgresql.service`, installs the backup
...
defect as a bundled Windows cluster, and the documentation calls that out explicitly. Then `Wicket.pkg`,
admin password once, whose postinstall lays down `com.wicket.server.plist` as a LaunchDaemon and
...
On the office server, `postgresql-x64-17` and `wicket-server` are both Windows Services set to Automatic
...
the tablet, and documented as an SOP because it is a Windows configuration rather than a Wicket feature.

At 6am the operator wakes the tablet and it is on the floor terminal screen. **Under five seconds to the
first scan.** The scan itself stays inside the 500ms budget partly because `wicket-server` connects to
...
`wicket-server` takes an automatic `wicket backup`, refuses to proceed if it fails, runs pending
migrations, and comes up. **Two to four minutes of downtime.** Linux:
`sudo apt install ./wicket_new.deb` and systemd restarts it — **about a minute**. macOS: the new `.pkg` —
**two minutes**.

The cluster is not touched. Only Wicket's own migrations run, and those are versioned and tested forward
...
change control** — and that is the correct owner. Wicket is not shipping two PostgreSQL major versions
...
Windows, concretely: run `wicket backup` and `wicket backup verify`, and keep the report. Install
PostgreSQL 19 with the EDB installer on a second port. Stop `wicket-server`. Run EDB's `pg_upgrade` — or
`pg_dumpall` and restore, which is slower, completely adequate at this data volume, and has a far
simpler failure mode. Update Wicket's configured port. Start Wicket. Run `wicket backup verify` against a
fresh backup, and run `wicket iq`. Uninstall the old major version once the verification passes, not
before.
...
Wicket's three obligations, which are the whole of our part: declare a supported PostgreSQL version range
per release; **never require a major bump inside a minor release**; and ship `wicket verify` plus the
...
1. New machine. Install PostgreSQL of the **same major version** recorded in the backup manifest, per
   (b) steps 1–2. **~10 min.**
2. Install Wicket at the **same minor version** recorded in the manifest. **~3 min.**
3. `wicket restore --from \\nas\wicket\2026-09-10.dump`. It creates the database, the roles and the
...
4. `verify` runs automatically and is not skippable: it re-computes the conservation check over every
   ...
   audit trail. **1–3 min.**
5. `wicket iq` runs the published installation qualification suite. **2–5 min.**
...
is the same line the regulatory work already drew — Wicket is Part 11-*capable*; only a deployment is
Part 11-*compliant*.
