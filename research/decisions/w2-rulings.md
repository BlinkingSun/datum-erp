...
- **§5a amended.** The raw-SQL fence confines only the session protocol (`set_config`, `current_setting`,
  `raw_sql`, `copy_in_raw`, `QueryBuilder`, `query!` macros) to `datum-db`, `datum-audit`, `datum-test`; the
...
- **§5a.1 — the fence is a law, not a grep.** Assembling a confined token at build or run time, generating
  ...
  merit. Measured on `datum-uom` attempt 1 (disqualified) and enforced since by `just lint-sql` greps
...
- **§2 workspace members** gain the `modules/*` glob so Wave 2s modules register by existing under
  `modules/<name>/`.
- **§7 recipes:** `db-reset` skips `*-gc.sql`; `db-gc` drops stale `datum_t_*` case databases (opt-in) using
  the harness's `COMMENT ON DATABASE` creation stamp.

## Batch rulings

- **2.1 datum-db / 2.2 datum-audit** — see the run record (Wave 1 close and batch 2.1/2.2 lines).
- **2.3 datum-identity (R-2.3-IDENTITY).** Split audit ruled FAIL with a scoped rework: built-ins are inserted by
...
- **2.3 datum-uom (R-2.3-UOM).** Attempt 1 disqualified for fence evasion. Attempt 2: the cursor lineage
...
- **2.4 datum-ledger (THE GATE).** Both attempts double-failed; ADDENDA 1–2 (18 items) consolidated every
...
- **2.5 datum-statemachine.** Head-to-head returned no winner (both skipped `finalize` on an after-hook error);
...
- **2.5 datum-jobs.** First audit failed on fence allows and schema class; the grok rework won (queue tables in
  `transient`, `run_log` in `app`); a Windows-only timing race in the progress test was replaced by a handshake.
- **2.6 datum-module.** The cursor attempt won (migrator order with `schema_history` attach and audit on every
