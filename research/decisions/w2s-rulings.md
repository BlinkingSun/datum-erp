...
| R-2s-2 | One name triple per module: crate `wicket-mod-<x>`, manifest id `mod-<x>` (profiles + compiled-in), schema `<x>`. 2s1-glue aligns the 2s.1 modules. | lots adjudication residual 2, items audit residual 3, locations audit residual 3 | 2026-09-12 |
...
| R-2s-8 | `SECURITY DEFINER` functions are allowed only in the migrations of `wicket-db`, `wicket-audit`, `wicket-ledger` and `wicket-numbering`; `just lint-sql` scans migrations and fails any other occurrence, any `wicket.*` GUC write outside wicket-db/wicket-audit (0001 preambles grandfathered), and a DROP that cancels a CREATE. | lint-migrations findings → lanes lint-migrations, lint-migrations-c1 | 2026-09-12 |
